...
class AttendanceImportCenter {
	...
	render_processing_results(rows = [], loading = false, meta = {}) {
		...
		const headers = isAttendanceDraft
			? ["序号"].concat(this.attendance_draft_columns().map(([, label]) => label), ["异常说明", "处理状态"])
			: isAppleTree
				? ["序号"].concat(this.apple_tree_columns().map(([, label]) => label), ["异常说明", "处理状态", "是否计入下游", "来源追溯"])
				: isMissedPunch
					? ["序号"].concat(this.missed_punch_columns().map(([, label]) => label), ["异常", "处理状态", "来源追溯"])
					: isMonthlySupport
						? ["序号"].concat(supportColumns.map(([, label]) => label), ["导入校验", "导入状态", "来源追溯"])
						: ["工号", "姓名", "部门", "加工结果", "异常", "处理状态", "来源追溯"];
		...
		const resultDescription = isAttendanceDraft
			...
			: isAppleTree
				? __("每条苹果树记录保留奖惩、审批与来源字段；姓名＋部门唯一匹配时自动补全工号。这里只显示无异常或已经处理的记录。")
				: isMissedPunch
					? __("每笔补卡审批完整展示；不能确定的记录需先在异常队列处理，处理结果会保留审计记录并同步到本页。")
					: isMonthlySupport
						? __("显示一次性导入的金额或逐日工时，并保留校验结果帮助判断文件是否正确导入。发现错误时请更正原文件后重新上传；不进入异常处理，也不支持二次加工。")
		...
		return `
			<div class="hrms-attendance-section">
				...
				${!loading && pendingExceptionCount ? `<div class="hrms-attendance-api-notice"><strong>${this.escape(__("仍有待处理异常"))}</strong><span>${this.escape(__("当前来源有 {0} 条异常待处理；请先到“异常处理”修改后再确认和下载本类加工结果。", [pendingExceptionCount]))}</span></div>` : ""}
	...
	render_attendance_draft_result_row(row, index) {
		...
		return `<tr><td>${this.escape(index + 1)}</td>${this.attendance_draft_columns().map(([field]) => `<td class="${["attendance_details", "exception_events"].includes(field) ? "hrms-attendance-long-cell" : ""}">${this.escape(displayValue(field))}</td>`).join("")}<td><strong>${this.escape(exception)}</strong>${detail}</td><td>${this.review_status_badge(row.review_status || "待审核")}</td></tr>`;
	}
						...
						primary_action: (values) => {
							...
							this.call_processing_api(
								...
								{ company: this.company, attendance_month: this.attendance_month, record_id: recordId, source_row: dailyRow.source_row, changes, review_status: values.review_status, reason: values.reason },
								{
									...
									on_success: () => { dialog.hide(); frappe.show_alert({ message: __("已保存每日修改，并已更新该员工的下游汇总。"), indicator: "green" }); this.load_processing_exceptions(); },
									on_error: (message) => frappe.msgprint(message),
								},
							);
						},
			...
			<section class="hrms-attendance-monthly-support">
				...
				<div class="hrms-attendance-monthly-support-grid">
					${this.monthly_support_sources.map((source) => {
						...
						const manualAction = source.key === "special_hours"
							? `<button class="btn btn-default btn-xs" data-special-hours-manual ${processedRows && check.status === "已就绪" ? "" : "disabled"}>${this.escape(__("手动修改"))}</button>`
							: "";
	...
	open_monthly_final_editor(kind = "signed") {
		this.call_processing_api("get_monthly_final_preview", {
			...
			on_success: (data) => {
				...
				const dialog = new frappe.ui.Dialog({
					title: __("编辑员工签字版（会同步重生成财务版）"),
					fields: [
						{ fieldtype: "HTML", fieldname: "final_editor", options: `<div class="hrms-attendance-dialog-note">${this.escape(__("此处修改的是员工签字版的同源数据。保存后会记录原值和操作人，当前锁定版将不再作为有效版本；请重新锁定生成两个版本。平日、周末、节假日特殊工时必须按日期在“特殊工时 → 手动修改”中维护，以保留倍率依据。"))}</div>${table}` },
					],
					size: "extra-large",
	...
	render_dingtalk_integration() {
		this.body().innerHTML = `
			<div class="hrms-attendance-section hrms-attendance-integration">
				...
				<div class="alert alert-info">${this.escape(__("钉钉数据不会直接改员工主档、月度锁定记录或薪资。先同步组织与员工并核对映射，再对指定日期和最多 5 名员工进行本地试运行，系统才会生成可复核的每日考勤草稿。"))}</div>
				...
				<div class="hrms-attendance-import-panel mt-3">
					...
					<p>${this.escape(__("1. 配置并测试连接 → 2. 同步组织和员工 → 3. 在员工映射中处理待匹配/冲突 → 4. 对指定日期和最多 5 人试运行 → 5. 查看草稿和异常；确认无误后才进入月度终稿。"))}</p>
					<div>
		...
		frappe.call({
			...
			callback: (response) => {
				...
				const dialog = new frappe.ui.Dialog({
					title: __("配置钉钉集成"),
					fields: [
						{ fieldtype: "HTML", options: `<div class="alert alert-info">${this.escape(__("本地试运行只需内网服务器主动访问钉钉；不会开放公网端口。Client Secret 仅保存到服务器密码字段，已保存时可留空。手动试运行成功并核对异常后，才勾选每日自动同步。"))}</div>` },
						...
						{ fieldname: "sync_mode", fieldtype: "Select", label: __("同步模式"), options: "Excel导入（默认）\n内网服务器主动拉取API", default: settings.sync_mode || defaults.sync_mode || "内网服务器主动拉取API", reqd: 1 },
						{ fieldname: "daily_sync_enabled", fieldtype: "Check", label: __("每日自动同步（02:30）"), description: __("同步昨天并回补最近 N 天；请在手动拉取核对通过后再启用。"), default: settings.daily_sync_enabled ? 1 : 0 },
						...
						{ fieldname: "client_id", fieldtype: "Data", label: __("Client ID / AppKey"), default: settings.client_id || defaults.client_id || "", reqd: 1 },
						{ fieldname: "client_secret", fieldtype: "Password", label: __("Client Secret / AppSecret"), description: result.client_secret_saved ? __("已保存；留空可保持不变。") : __("首次连接必填；不会显示、不会写入代码。") },
					],
