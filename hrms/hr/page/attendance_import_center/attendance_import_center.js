...
class AttendanceImportCenter {
	constructor(page) {
		...
		this.daily_attendance_data = null;
		this.daily_attendance_date = "";
		this.selected_source_type = "attendance_draft";
		this.exception_source_filter = "";
		this.exception_employee_code_filter = "";
		this.exception_employee_name_filter = "";
		this.exception_department_filter = "";
		this.exception_code_filter = "";
		this.exception_processing_status_filter = "";
		this.exception_department_options = [];
		this.exception_search_timer = null;
		this.exception_sort_field = "employee_code";
		this.exception_sort_order = "asc";
		...
		this.processing_statuses = ["未上传", "待加工", "导入异常", "待处理异常", "待确认", "已确认"];
		this.workflow_views = [
			{ key: "exceptions", label: "异常处理" },
			{ key: "processing-results", label: "加工结果" },
			{ key: "monthly-final", label: "月度终稿" },
			{ key: "daily-review", label: "日考勤审核" },
			{ key: "daily-attendance", label: "日考勤" },
		];
		this.view_groups = [
			{
				title: "考勤处理",
			...
			},
			{
				title: "规则设置",
				items: [
					{ key: "schedule-management", label: "排班管理" },
		...
		// 钉钉是跨考勤、组织和审批的独立集成入口，只从顶部“更多”进入，
		// 不与日常考勤处理左侧导航混在一起。
		this.standalone_views = [{ key: "dingtalk", label: "钉钉集成" }];
		this.route_aliases = {
			"summary": "monthly-final",
			"import": "monthly-final",
			"daily-import": "daily-attendance",
			...
			"shift-rules": "schedule-management",
		...
		this.view_map = [...this.view_groups.flatMap((group) => group.items), ...this.standalone_views].reduce((map, item) => {
			map[item.key] = item;
			return map;
		}, {});
		this.active_view = this.resolve_view(frappe.get_route()[1] || "monthly-final");
	...
	view_from_current_route() {
		...
		return this.resolve_view(route[1] || "monthly-final");
	}
	...
	view_from_route_detail(detail) {
		...
		return this.resolve_view(parts[1] || "monthly-final");
	}
	...
	resolve_view(view) {
		...
		return this.view_map[normalized] || this.workflow_views.find((item) => item.key === normalized) ? normalized : "monthly-final";
	}
	...
	render_header() {
		const isDingtalk = this.active_view === "dingtalk";
		const isCompanyRuleView = this.active_view === "schedule-management";
		return `
			<div class="hrms-attendance-import-head">
				<div>
					<h2>${this.escape(__(isDingtalk ? "钉钉集成" : isCompanyRuleView ? "公司考勤规则" : "考勤处理中心"))}</h2>
				</div>
				<div class="hrms-attendance-import-controls">
					...
					${isCompanyRuleView ? "" : this.render_month_control()}
				</div>
			</div>
		`;
	}
	...
	render_kpi_grid() {
		if (["dingtalk", "schedule-management"].includes(this.active_view)) return "";
		const batch = this.processing_batch || {};
		const slots = batch.slots || [];
		const confirmed = slots.filter((slot) => slot.status === "已确认").length;
		const processed = slots.filter((slot) => !["未上传", "待加工", "导入异常"].includes(slot.status)).length;
		const exceptions = slots.reduce((total, slot) => total + Number(slot.exception_count || 0), 0);
		const cards = [
			["待处理异常", `${exceptions}条`, "pending-exceptions", "exceptions"],
			["加工结果", `${processed}/3 已生成`, "processing-results", "processing-results"],
			["月度考勤", batch.final_outputs?.locked_version ? "已生成" : "待生成", "monthly-attendance", "monthly-final"],
		];
		return `
			<div class="hrms-attendance-kpi-grid hrms-attendance-overview-grid">
				${cards
					.map(
						([label, value, key, view]) => `
							<button class="hrms-attendance-kpi" data-view="${this.escape(view)}">
								<strong>${this.escape(__(label))}</strong>
								<span data-kpi="${key}">${this.escape(__(value))}</span>
							</button>
						`,
					)
					.join("")}
	...
	update_processing_kpis() {
		...
		const values = {
			...
			"processing-results": `${slots.filter((slot) => !["未上传", "待加工", "导入异常"].includes(slot.status)).length}/3 已生成`,
			"monthly-attendance": batch.final_outputs?.locked_version ? "已生成" : "待生成",
		};
	...
	capture_exception_scroll_position(recordId = "", sourceRow = "", sourceContext = {}) {
		return { windowY: window.scrollY || document.documentElement.scrollTop || 0,
			recordId, sourceRow: String(sourceRow), sourceFile: sourceContext.sourceFile || "",
			sourceSheet: sourceContext.sourceSheet || "", attendanceDate: sourceContext.attendanceDate || "" };
	}

	exception_scroll_target(position) {
		if (!position.recordId) return null;
		const cards = Array.from(this.body()?.querySelectorAll("[data-exception-card-record]") || [])
			.filter((card) => card.dataset.exceptionCardRecord === position.recordId);
		return cards.find((card) => card.dataset.attendanceSourceRow === position.sourceRow
			&& card.dataset.attendanceSourceFile === position.sourceFile
			&& card.dataset.attendanceSourceSheet === position.sourceSheet
			&& card.dataset.attendanceDate === position.attendanceDate)
			|| cards.find((card) => card.dataset.attendanceDate >= position.attendanceDate)
			|| cards[0] || null;
	}

	restore_exception_scroll_position(position) {
		if (!position) return;
		requestAnimationFrame(() => requestAnimationFrame(() => {
			if (this.active_view !== "exceptions") return;
			const card = this.exception_scroll_target(position);
			if (card) {
				card.style.scrollMarginTop = "90px";
				card.scrollIntoView({ block: "start", behavior: "instant" });
			} else window.scrollTo(0, Number(position.windowY) || 0);
		}));
	}
	...
	load_active_view() {
		...
		// Every view shares the company/month KPI header. Non-final views used to
		// load only their own table, leaving real historical batches displayed as
		// "读取中 / 0/3" indefinitely.
		if (this.active_view !== "monthly-final" && this.active_view !== "daily-import" && this.active_view !== "daily-attendance") {
			const contextKey = `${this.company || ""}|${this.attendance_month || ""}`;
			if (!this.processing_batch || this.processing_batch_context_key !== contextKey) this.load_dashboard_summary();
			else this.update_processing_kpis();
		}
		if (this.active_view === "daily-review") return this.load_daily_review_queue();
		if (this.active_view === "daily-import" || this.active_view === "daily-attendance") return this.load_daily_attendance();
		if (this.active_view === "processing-results") return this.load_processing_results();
		// Re-entering the page may reuse the same company/month/filter snapshot.
		// Mutations and filter changes call the loader without this flag and refresh it.
		if (this.active_view === "exceptions") return this.load_processing_exceptions({ reuseSnapshot: true });
		...
		if (this.active_view === "schedule-management") return this.load_attendance_shift_rules();
	...
	normalize_processing_batch(data = {}) {
		...
		return {
			...
			first_signed_inputs: data.first_signed_inputs || [],
			first_signed_outputs: data.first_signed_outputs || {},
	...
	load_processing_batch({ rerender_active = true, on_loaded } = {}) {
		...
		const requestedMonth = this.attendance_month;
		this.call_processing_api(
			"get_processing_batch",
			{ company: requestedCompany, attendance_month: requestedMonth },
			{
				on_success: (data) => {
					if (requestedCompany !== this.company || requestedMonth !== this.attendance_month) return;
					this.processing_batch = this.normalize_processing_batch(data);
					this.processing_batch_context_key = `${requestedCompany}|${requestedMonth}`;
					this.update_processing_kpis();
	...
	render_current_processing_view() {
		if (this.active_view === "daily-import" || this.active_view === "daily-attendance") return this.render_daily_attendance();
	...
	review_status_badge(status) {
		...
		return `<span class="hrms-attendance-processing-status ${className}">${this.escape(__(label))}</span>`;
	}

	load_daily_import() {
		this.load_daily_attendance();
	}

	render_daily_import() {
		this.render_daily_attendance();
	}

	load_daily_review_queue() {
		const body = this.body();
		if (!body) return;
		body.innerHTML = this.render_action_bar("日考勤审核", [
			{ label: "刷新审核队列", action: "refresh", primary: true },
		]);
		body.querySelector("[data-action='refresh']")?.addEventListener("click", () => this.load_daily_review_queue());
		body.querySelector("[data-table]").innerHTML = `<div class="hrms-attendance-review-panel" data-daily-review-summary><div><strong>${this.escape(__("人事审核队列"))}</strong><span>${this.escape(__("正在读取同步后的日考勤日期..."))}</span></div></div>`;
		if (!this.ensure_company()) return;
		this.call_processing_api("list_daily_attendance_review_queue", {
			company: this.company,
			attendance_month: this.attendance_month,
		}, {
			on_success: (data) => this.render_daily_review_queue(data || {}),
			on_error: (message) => {
				const table = this.body()?.querySelector("[data-table]");
				if (table) table.innerHTML = `<div class="hrms-attendance-api-notice"><strong>${this.escape(__("读取失败"))}</strong><span>${this.escape(message)}</span></div>`;
			},
		});
	}

	render_daily_review_queue(data = {}) {
		const body = this.body();
		const table = body?.querySelector("[data-table]");
		if (!table) return;
		const items = data.items || [];
		const summary = body.querySelector("[data-daily-review-summary]");
		if (summary) {
			summary.querySelector("div span").textContent = __("共 {0} 个考勤日，待审核 {1} 个，已锁定 {2} 个；记录 {3} 条，待处理异常 {4} 条，未匹配 {5} 条。", [data.total_dates || 0, data.pending_dates || 0, data.locked_dates || 0, data.total_rows || 0, data.pending_exception_rows || 0, data.unmatched_rows || 0]);
		}
		const actionFor = (item) => {
			const actions = [`<button class="btn btn-default btn-xs" data-daily-review-open="${this.escape(item.attendance_date)}">${this.escape(__("查看明细"))}</button>`];
			if (item.can_validate && !item.locked) actions.push(`<button class="btn btn-default btn-xs" data-daily-review-validate="${this.escape(item.attendance_date)}">${this.escape(__("修改后校验"))}</button>`);
			if (item.can_lock && !item.locked) actions.push(`<button class="btn btn-primary btn-xs" data-daily-review-close="${this.escape(item.attendance_date)}">${this.escape(__("审核通过并锁定"))}</button>`);
			return actions.join(" ");
		};
		table.innerHTML = `<div class="hrms-attendance-table-wrap"><table class="table table-bordered hrms-attendance-table"><thead><tr><th>${this.escape(__("考勤日期"))}</th><th>${this.escape(__("记录"))}</th><th>${this.escape(__("已匹配"))}</th><th>${this.escape(__("待处理异常"))}</th><th>${this.escape(__("未匹配"))}</th><th>${this.escape(__("审核状态"))}</th><th>${this.escape(__("操作"))}</th></tr></thead><tbody>${items.length ? items.map((item) => `<tr><td>${this.escape(item.attendance_date)}</td><td>${this.escape(item.total_rows || 0)}</td><td>${this.escape(item.matched_rows || 0)}</td><td>${this.escape(item.pending_exception_rows || 0)}</td><td>${this.escape(item.unmatched_rows || 0)}</td><td>${this.status_badge(item.status || "待校验")}<br><small class="text-muted">${this.escape(item.validation_message || "")}</small></td><td>${actionFor(item)}</td></tr>`).join("") : `<tr><td colspan="7" class="text-muted">${this.escape(__("当前月份还没有同步后的日考勤记录；同步完成但接收数为 0 时不会生成审核日期。"))}</td></tr>`}</tbody></table></div>`;
		table.querySelectorAll("[data-daily-review-open]").forEach((button) => button.addEventListener("click", () => {
			this.daily_attendance_date = button.dataset.dailyReviewOpen;
			this.attendance_date = button.dataset.dailyReviewOpen;
			this.set_view("daily-attendance");
		}));
		table.querySelectorAll("[data-daily-review-validate]").forEach((button) => button.addEventListener("click", () => this.validate_daily_review_date(button.dataset.dailyReviewValidate)));
		table.querySelectorAll("[data-daily-review-close]").forEach((button) => button.addEventListener("click", () => this.close_daily_review_date(button.dataset.dailyReviewClose)));
	}

	validate_daily_review_date(attendanceDate) {
		if (!this.ensure_company() || !attendanceDate) return;
		this.call_processing_api("validate_daily_attendance_after_review", { company: this.company, attendance_month: this.attendance_month, attendance_date: attendanceDate }, {
			freeze: true,
			freeze_message: __("正在校验修改后的员工匹配、异常和来源版本..."),
			on_success: (data) => {
				frappe.show_alert({ message: data.validation_message || __("日考勤校验已完成。"), indicator: data.validation_status === "通过" ? "green" : "orange" });
				this.load_daily_review_queue();
			},
			on_error: (message) => frappe.msgprint(message),
		});
	}

	close_daily_review_date(attendanceDate) {
		if (!this.ensure_company() || !attendanceDate) return;
		const dialog = new frappe.ui.Dialog({
			title: __("审核通过并锁定当天考勤"),
			fields: [{ fieldtype: "HTML", options: `<div class="hrms-attendance-dialog-note">${this.escape(__("锁定 {0} 后，月度考勤只能使用这一天的审核版本；后续修改必须走重新同步或修订流程。", [attendanceDate]))}</div>` }, { fieldname: "remarks", fieldtype: "Small Text", label: __("审核备注"), reqd: 1 }],
			primary_action_label: __("确认锁定当天"),
			primary_action: (values) => this.call_processing_api("close_daily_attendance", { company: this.company, attendance_month: this.attendance_month, attendance_date: attendanceDate, remarks: values.remarks || "" }, {
				freeze: true,
				freeze_message: __("正在锁定当天考勤并记录审核人..."),
				on_success: (data) => {
					dialog.hide();
					frappe.show_alert({ message: __("{0} 已审核通过并锁定。", [data.attendance_date]), indicator: "green" });
					this.load_daily_review_queue();
				},
				on_error: (message) => frappe.msgprint(message),
			}),
	...
	load_daily_attendance() {
		const body = this.body();
		if (!body) return;
		body.innerHTML = this.render_daily_attendance_markup({}, true);
		if (!this.ensure_company()) return;
		const requestedCompany = this.company;
		const requestedMonth = this.attendance_month;
		this.call_processing_api("list_daily_attendance_records", {
			company: requestedCompany,
			attendance_month: requestedMonth,
			attendance_date: this.daily_attendance_date,
		}, {
			on_success: (data) => {
				if (requestedCompany !== this.company || requestedMonth !== this.attendance_month) return;
				this.daily_attendance_data = data;
				this.render_daily_attendance();
			},
			on_error: (message) => {
				if (requestedCompany !== this.company || requestedMonth !== this.attendance_month) return;
				this.daily_attendance_data = { items: [], available_dates: [], notice: message, error: message };
				this.render_daily_attendance();
			},
		});
	}

	render_daily_attendance_markup(data = {}, loading = false) {
		const rows = data.items || [];
		const dates = data.available_dates || [];
		const headers = ["日期", "部门", "员工", "工号", "日期类型", "班次", "上班", "下班", "标准工时", "实际出勤", "工作日加班", "休息日加班", "节假日加班", "缺卡", "审核/更正"];
		const renderRow = (row) => `<tr><td>${this.escape(row.attendance_date)}</td><td>${this.escape(row.department || "--")}</td><td>${this.escape(row.employee_name || "--")}</td><td>${this.escape(row.employee_code || "--")}</td><td>${this.escape(row.date_type || "--")}</td><td>${this.escape(row.shift || "--")}</td><td>${this.escape(row.clock_in || "--")}</td><td>${this.escape(row.clock_out || "--")}</td><td>${this.escape(row.standard_hours)}</td><td>${this.escape(row.actual_attendance_hours)}</td><td>${this.escape(row.workday_overtime_hours)}</td><td>${this.escape(row.restday_overtime_hours)}</td><td>${this.escape(row.holiday_overtime_hours)}</td><td>${this.escape(`${row.clock_in_missing || 0}/${row.clock_out_missing || 0}`)}</td><td>${row.name ? `<button class="btn btn-default btn-xs" data-daily-day-check="${this.escape(row.name)}">${this.escape(__("核对/人工更正"))}</button>` : "--"}</td></tr>`;
		const excludedNotice = Number(data.excluded_missing_employee_code_rows || 0)
			? `<div class="hrms-attendance-api-notice"><strong>${this.escape(__("已排除 {0} 条非员工账号记录", [data.excluded_missing_employee_code_rows]))}</strong><span>${this.escape(__("这些行缺少工号，可能是车间/设备等共享账号；不会显示为员工，也不会进入考勤初稿异常。"))}</span></div>`
			: "";
		const workflow = data.daily_workflow || {};
		const selectedDate = this.daily_attendance_date || "";
		const workflowLabel = workflow.required === false ? "当前来源尚未形成日考勤闭环记录" : workflow.locked ? "当天已锁定" : workflow.validation_status === "通过" ? "修改后校验通过，待锁定" : workflow.validation_status === "存在异常" ? `仍有 ${workflow.unmatched_rows || 0} 条未匹配、${workflow.pending_exception_rows || 0} 条待处理异常` : "待执行修改后校验";
		const dailyWorkflow = selectedDate ? `<section class="hrms-attendance-daily-closure"><div><strong>${this.escape(__("日考勤闭环"))}</strong><span>${this.escape(__(workflowLabel))}</span>${workflow.total_rows ? `<small>${this.escape(__("记录 {0} 条；已匹配 {1} 条；修改 {2} 条", [workflow.total_rows, workflow.matched_rows || 0, workflow.corrected_rows || 0]))}</small>` : ""}</div><div><button class="btn btn-default btn-sm" data-daily-validate ${workflow.can_validate ? "" : "disabled"}>${this.escape(__("校验修改后数据"))}</button> <button class="btn btn-primary btn-sm" data-daily-close ${workflow.can_lock ? "" : "disabled"}>${this.escape(__("审核通过并锁定当天"))}</button></div></section>` : `<section class="hrms-attendance-daily-closure"><div><strong>${this.escape(__("日考勤闭环"))}</strong><span>${this.escape(__(workflowLabel))}</span><small>${this.escape(__("请选择日期后执行修改后校验；已锁定日期才允许进入月考勤。"))}</small></div></section>`;
		const syncLabel = selectedDate && workflow.total_rows ? "重新同步钉钉数据" : "同步钉钉考勤";
		const dateOptions = dates.map((value) => `<option value="${this.escape(value)}"></option>`).join("");
		const dateClearButton = this.daily_attendance_date ? ` <button type="button" class="btn btn-link btn-sm" data-daily-date-clear>${this.escape(__("全部日期"))}</button>` : "";
		return `<div class="hrms-attendance-section"><div class="hrms-attendance-list-head"><div><h3>${this.escape(__("日考勤"))}</h3><small>${this.escape(__("日考勤来源、修改审批和修改后校验在这里完成；原始钉钉数据不会被覆盖。"))}</small></div><div><button class="btn btn-primary btn-sm" data-daily-sync>${this.escape(__(syncLabel))}</button> <button class="btn btn-default btn-sm" data-daily-upload>${this.escape(__(data.source_file_name ? "更新日考勤文件" : "导入日考勤文件"))}</button> <button class="btn btn-default btn-sm" data-daily-open-summary ${data.source_file_name ? "" : "disabled"}>${this.escape(__("进入考勤汇总加工"))}</button></div></div>${data.notice ? `<div class="hrms-attendance-api-notice"><strong>${this.escape(data.error ? __("读取失败") : __("提示"))}</strong><span>${this.escape(data.notice)}</span></div>` : ""}${excludedNotice}${dailyWorkflow}<div class="hrms-attendance-result-controls"><label>${this.escape(__("查看日期"))}<input type="date" class="form-control input-sm" data-daily-date value="${this.escape(this.daily_attendance_date || "")}" list="daily-attendance-date-options" /></label><datalist id="daily-attendance-date-options">${dateOptions}</datalist>${dateClearButton}<span class="text-muted">${this.escape(data.source_file_name ? __("来源：{0}；当前显示 {1}/{2} 条", [data.source_file_name, rows.length, data.total_count ?? rows.length]) : __("尚未导入当月日考勤文件。"))}</span></div><div class="hrms-attendance-table-wrap"><table class="table table-bordered hrms-attendance-table"><thead><tr>${headers.map((header) => `<th>${this.escape(__(header))}</th>`).join("")}</tr></thead><tbody>${loading ? `<tr><td colspan="${headers.length}" class="text-muted">${this.escape(__("正在读取日考勤明细..."))}</td></tr>` : rows.length ? rows.map(renderRow).join("") : `<tr><td colspan="${headers.length}" class="text-muted">${this.escape(__(data.source_file_name ? "当前筛选没有日考勤记录。" : "请先导入钉钉每日明细文件。"))}</td></tr>`}</tbody></table></div></div>`;
	}

	render_daily_attendance() {
		const body = this.body();
		if (!body) return;
		body.innerHTML = this.render_daily_attendance_markup(this.daily_attendance_data || {}, false);
		body.querySelector("[data-daily-sync]")?.addEventListener("click", () => this.sync_dingtalk_attendance_for_date());
		body.querySelector("[data-daily-upload]")?.addEventListener("click", () => this.open_slot_uploader("attendance_draft"));
		body.querySelector("[data-daily-open-summary]")?.addEventListener("click", () => this.set_view("monthly-final"));
		body.querySelectorAll("[data-daily-day-check]").forEach((button) => button.addEventListener("click", () => this.open_day_check_review(button.dataset.dailyDayCheck)));
		body.querySelector("[data-daily-validate]")?.addEventListener("click", () => this.validate_daily_attendance_day());
		body.querySelector("[data-daily-close]")?.addEventListener("click", () => this.close_daily_attendance_day());
		body.querySelector("[data-daily-date]")?.addEventListener("change", (event) => {
			this.daily_attendance_date = event.currentTarget.value;
			this.load_daily_attendance();
		});
		body.querySelector("[data-daily-date-clear]")?.addEventListener("click", () => {
			this.daily_attendance_date = "";
			this.load_daily_attendance();
		});
	}

	validate_daily_attendance_day() {
		if (!this.ensure_company() || !this.daily_attendance_date) return;
		this.call_processing_api("validate_daily_attendance_after_review", {
			company: this.company,
			attendance_month: this.attendance_month,
			attendance_date: this.daily_attendance_date,
		}, {
	...
	close_daily_attendance_day() {
		if (!this.ensure_company() || !this.daily_attendance_date) return;
		const dialog = new frappe.ui.Dialog({
			title: __("审核通过并锁定当天考勤"),
			fields: [{ fieldtype: "Small Text", fieldname: "remarks", label: __("审核备注"), description: __("锁定后当天数据不能普通修改；后续修订必须新增版本。") }],
			primary_action_label: __("确认锁定当天"),
			primary_action: (values) => this.call_processing_api("close_daily_attendance", {
				company: this.company,
				attendance_month: this.attendance_month,
				attendance_date: this.daily_attendance_date,
				...
				on_success: (data) => {
					dialog.hide();
					frappe.show_alert({ message: __("{0} 已锁定，月考勤将使用该版本。", [data.attendance_date]), indicator: "green" });
					this.load_daily_attendance();
				},
				on_error: (message) => frappe.msgprint(message),
			}),
	...
	render_attendance_summary_markup(batch = this.processing_batch) {
		return `
			<section class="hrms-attendance-section">
				<div class="hrms-attendance-list-head">
					<div><h3>${this.escape(__("考勤汇总"))}</h3></div>
					<div><button class="btn btn-primary btn-sm" data-bulk-source-upload>${this.escape(__("批量导入"))}</button></div>
				</div>
	...
	bind_attendance_summary_events(body) {
	...
	open_bulk_source_uploader() {
		...
		const requiredSources = [...this.processing_sources, ...this.monthly_support_sources];
		const dialog = new frappe.ui.Dialog({
			title: __("批量导入考勤文件"),
			fields: [{ fieldname: "bulk_upload", fieldtype: "HTML" }],
			size: "large",
		});
		const renderUploadDialog = () => {
				...
				<div class="hrms-attendance-bulk-upload">
					...
					<div class="hrms-attendance-bulk-upload__files">
						${selectedFiles.length
							? `<ol>${selectedFiles.map((file) => `<li title="${this.escape(file.file_name || file.file_url)}">${this.escape(file.file_name || file.file_url)}</li>`).join("")}</ol>`
							: `<span class="text-muted">${this.escape(__("尚未选择文件。"))}</span>`}
					</div>
			...
			`);
			dialog.$wrapper.find("[data-bulk-select-files]").on("click", () => {
				new frappe.ui.FileUploader({
					...
					on_success: (file) => {
						...
						uploaded.set(file.file_url, { file_url: file.file_url, file_name: file.file_name || file.name || "" });
						renderUploadDialog();
					},
				});
			});
			dialog.$wrapper.find("[data-bulk-clear-files]").on("click", () => {
				uploaded.clear();
				renderUploadDialog();
			});
	...
	bulk_import_and_process_sources(files) {
		this.call_processing_api("bulk_import_and_process_sources", {
			company: this.company,
			attendance_month: this.attendance_month,
			...
			on_success: (data) => {
				...
				const processed = items.map((item) => `${item.label}：${item.processed_rows || 0}条`).join("；");
				frappe.show_alert({ message: `${data.notice || __("已完成批量导入和加工。")} ${processed}；${__("待处理异常 {0} 条。", [exceptions])}`, indicator: exceptions ? "orange" : "green" });
	...
	render_processing_slot(slot, loading = false) {
		...
		const openTarget = "processing-results";
		const openLabel = __("查看加工结果");
	...
	open_slot_uploader(sourceType) {
		if (!this.ensure_company()) return;
		const selectedSourceType = sourceType || "attendance_draft";
		this.selected_source_type = selectedSourceType;
		new frappe.ui.FileUploader({
			folder: "Home/Attachments",
			restrictions: { allowed_file_types: [".xlsx"] },
			on_success: (file) => {
				this.call_processing_api(
					...
					{ company: this.company, attendance_month: this.attendance_month, source_type: selectedSourceType, file_url: file.file_url },
					{
						freeze: true,
						freeze_message: __("正在登记来源文件..."),
						on_success: () => {
							frappe.show_alert({ message: __("文件已登记，正在自动加工并检查。"), indicator: "blue" });
							this.run_slot_action("process_source_slot", selectedSourceType);
						},
	...
	run_slot_action(method, sourceType) {
		...
		this.call_processing_api(
			...
			{ company: this.company, attendance_month: this.attendance_month, source_type: sourceType },
			{
				...
				on_success: () => {
					frappe.show_alert({ message: method === "precheck_source_slot" ? __("结构预检完成。") : __("加工完成；请在本页确认，或先处理异常。"), indicator: "green" });
					if (this.active_view !== "monthly-final") this.set_view("monthly-final");
					else this.load_monthly_final();
				},
				on_error: (message) => frappe.msgprint(message),
			},
		);
	...
	load_processing_results() {
		...
		this.call_processing_api(
			"list_processing_results",
			{
				...
				page_start: (this.processing_result_page - 1) * this.processing_result_page_size,
				page_length: this.processing_result_page_size,
			},
			{
				on_success: (data) => {
					const allRows = data.processed_rows || [];
					const totalPages = Math.max(1, Math.ceil(Number(data.total_count || 0) / this.processing_result_page_size));
					if (this.processing_result_page > totalPages) {
						this.processing_result_page = totalPages;
						this.load_processing_results();
						return;
					}
					body.innerHTML = this.render_processing_results(allRows, false, data);
					this.bind_processing_result_events();
				},
				on_error: (message) => {
	...
	render_processing_result_pagination(totalCount = 0) {
		const pageCount = Math.max(1, Math.ceil(Number(totalCount || 0) / this.processing_result_page_size));
		const page = Math.min(this.processing_result_page, pageCount);
		const start = totalCount ? (page - 1) * this.processing_result_page_size + 1 : 0;
		const end = Math.min(page * this.processing_result_page_size, totalCount);
		return `<div class="hrms-attendance-pagination"><small>${this.escape(__("显示第 {0}-{1} 条，共 {2} 条", [start, end, totalCount]))}</small><div class="hrms-attendance-pagination-controls"><button class="btn btn-default btn-sm" data-processing-result-page="${page - 1}" ${page <= 1 ? "disabled" : ""}>${this.escape(__("上一页"))}</button><span>${this.escape(__("第 {0} / {1} 页", [page, pageCount]))}</span><button class="btn btn-default btn-sm" data-processing-result-page="${page + 1}" ${page >= pageCount ? "disabled" : ""}>${this.escape(__("下一页"))}</button></div></div>`;
	}

	render_processing_source_tabs() {
		return `<div class="hrms-attendance-result-tabs">${this.processing_result_sources.map((source) => `<button class="btn btn-default btn-sm ${source.key === this.selected_source_type ? "active" : ""}" data-result-source="${this.escape(source.key)}">${this.escape(__(source.label))}</button>`).join("")}</div>`;
	}

	processing_source_label(sourceType) {
		return this.processing_result_sources.find((source) => source.key === sourceType)?.label || __("其他来源");
	}

	processing_field_label(fieldName) {
		if (String(fieldName || "").startsWith("__daily_exception_decision__:")) return __("按日确认休息日打卡");
		if (String(fieldName || "").startsWith("special_hours_days")) return __("休息日加班工时");
		const labels = {
			employee_code: "工号", employee_name: "姓名", department: "部门", standard_hours: "标准工时", actual_attendance_hours: "实际出勤工时",
			bereavement_leave_hours: "丧假工时", marriage_leave_hours: "婚假工时", public_leave_hours: "公假工时", maternity_leave_hours: "产假工时", reunion_leave_hours: "团圆假工时", leave_hours: "有效请假合计",
			...
			deep_night_shifts: "深夜班次数", large_night_shifts: "大夜班次数", small_night_shifts: "小夜班次数", personal_leave_hours: "事假工时", sick_leave_hours: "病假工时",
			annual_leave_hours: "特休工时", work_injury_hours: "工伤工时", rest_arrangement_hours: "排休工时", absence_hours: "旷工工时",
			absence_marker_count: "旷工标记次数", late_count: "迟到次数", early_count: "早退次数",
			...
			day: "日期", hours: "工时",
			attendance_details: "异常日期", attendance_date: "考勤日期", decision: "处理决定", shift: "班次", clock_in: "上班打卡", clock_out: "下班打卡",
		...
		return labels[fieldName] || (fieldName && !String(fieldName).startsWith("__") ? fieldName : __("其他调整字段"));
	}
	...
	manual_adjustment_changes(row) {
		const fieldName = String(row.field_name || "");
		const original = row.original_value;
		const modified = row.new_value;
		if (["__attendance_policy_recheck__", "__source_parser_repair__", "__review_decision__"].includes(fieldName) || fieldName.startsWith("__daily_exception_decision__:")) return [];
		if (fieldName.startsWith("special_hours_days:")) {
			const hoursValue = (value) => value && typeof value === "object" && !Array.isArray(value) && Object.prototype.hasOwnProperty.call(value, "hours") ? (value.hours ?? 0) : (value ?? 0);
			const before = hoursValue(original);
			const after = hoursValue(modified);
			return this.format_processing_value(before) === this.format_processing_value(after) ? [] : [{ label: __("休息日加班工时"), original: before, modified: after }];
		}
		if (original && modified && typeof original === "object" && typeof modified === "object" && !Array.isArray(original) && !Array.isArray(modified)) {
			const keys = [...new Set([...Object.keys(original), ...Object.keys(modified)])];
			const changed = keys.filter((key) => this.format_processing_value(original[key]) !== this.format_processing_value(modified[key]));
			if (changed.length) {
		...
		if (this.format_processing_value(original) === this.format_processing_value(modified)) return [];
	...
	attendance_draft_columns() {
		return [
			...
			["holiday_overtime_hours", "节假日加班"], ["deep_night_shifts", "深夜班"], ["large_night_shifts", "大夜班"], ["small_night_shifts", "小夜班"],
			["personal_leave_hours", "事假"], ["sick_leave_hours", "病假"], ["annual_leave_hours", "特休"], ["work_injury_hours", "工伤"],
			["bereavement_leave_hours", "丧假"], ["marriage_leave_hours", "婚假"], ["public_leave_hours", "公假"], ["maternity_leave_hours", "产假"], ["reunion_leave_hours", "团圆假"], ["leave_hours", "有效请假合计"],
	...
	attendance_detail_text(row) {
		const values = this.processing_values(row);
		const details = values.attendance_details;
		if (Array.isArray(details) && details.length) {
			return [...new Set(details.map((item) => item.attendance_date).filter(Boolean))].join("、") || "--";
		}
		if (row.source_type === "missing_card") return String(values.punch_time || "").slice(0, 10) || "--";
		if (row.source_type === "apple_tree") return values["奖惩日期"] || row.processed_value?.["奖惩日期"] || "--";
		return "--";
	}

	attendance_exception_lines(row) {
		const savedLines = row.daily_exception_lines || this.processing_values(row).exception_lines;
		if (Array.isArray(savedLines) && savedLines.length) return savedLines;
		const activeCodes = new Set(row.exception_codes || []);
		const fields = [
		...
		return details.reduce((lines, detail) => {
			const exceptionCodes = fields
				.filter(([code, field]) => activeCodes.has(code) && isPositive(detail?.[field]))
				.map(([code]) => code);
			if (exceptionCodes.length) lines.push({ ...detail, exception_codes: exceptionCodes });
			return lines;
		}, []);
	}
	...
	attendance_exception_date_text(row) {
		...
		if (row.source_type === "attendance_draft") {
			...
			return [...new Set(dates)].map((value) => this.attendance_date_with_weekday(value)).join("、") || "--";
		}
		if (dates.length) return [...new Set(dates)].map((value) => this.attendance_date_with_weekday(value)).join("、");
		return this.attendance_detail_text(row);
	}

	render_attendance_exception_lines(lines, recordId = "", includeAction = true) {
		const sourceExceptionLabel = (code) => ({
			...
			EARLY_MARKED: __("早退（钉钉标记）"), ABSENCE_MARKED: __("旷工（钉钉标记）"),
				...
				RESTDAY_CLOCKED_WITHOUT_APPROVAL: __("间接人员周末打卡缺加班单"),
				WORKDAY_OUTSIDE_SHIFT_UNAPPROVED: __("钉钉加班为0：缺加班单"),
				WORKDAY_OVERTIME_APPROVAL_NOT_APPLIED: __("钉钉加班为0：审批未生效"),
				WORKDAY_OVERTIME_DINGTALK_ANOMALY: __("钉钉加班为0：免审批计算异常"),
		...
		return lines.map((line) => {
			...
			const action = includeAction && recordId && !line.resolved && line.review_status !== "已处理异常" ? `<br>
				<button class="btn ${restdayWithoutOvertime || restdayWithoutApproval ? "btn-primary" : "btn-default"} btn-xs" data-edit-attendance-daily-row="${this.escape(recordId)}" ${locator} data-restday-overtime-correction="${restdayWithoutOvertime ? "1" : ""}">${this.escape(__(restdayWithoutOvertime ? "填写休息日加班时长" : restdayWithoutApproval ? "补充/核对加班单" : "修改本日"))}</button>
			...
			const balanceText = `实际出勤 ${line.actual_attendance_hours} ＋ 事假 ${line.personal_leave_hours || 0} ＋ 病假 ${line.sick_leave_hours || 0} ÷ 2 ＋ 团圆假 ${line.reunion_leave_hours || 0} ＋ 排休 ${line.rest_arrangement_hours || 0} ＋ 旷工 ${line.absence_hours || 0} ＝ ${hours(line.accounted_hours)}`;
			const comparison = hoursMismatch && line.accounted_hours !== undefined
				? resolved && difference === 0
					? `<br><strong>${this.escape(`处理后校验：${balanceText}，与标准 ${hours(line.standard_hours)} 已一致`)}</strong>`
					: `<br><strong>${this.escape(`输入不成立：${balanceText}，与标准 ${hours(line.standard_hours)} 不符；${difference < 0 ? "不足" : "超出"} ${hours(Math.abs(difference))}`)}</strong>`
				: "";
			const weekendRule = line.restday_clocked_without_approval
				? `间接人员周末打卡须加班单；依据班次：${line.indirect_restday_rule_shift || "待核对"}（${line.indirect_restday_rule_date || "日期待核对"}）`
				: line.is_weekend ? "周末，只核对时长" : line.date_type || "日期类型未提供";
			const facts = `<br><small>${this.escape(`班次：${line.shift || "--"}；计划：${line.scheduled_start || "待复核"} → ${line.scheduled_end || "待复核"}；${weekendRule}`)}<br>${this.escape(`实际打卡：${line.clock_in || "--"} → ${line.clock_out || "--"}；班次外原始时长：${hours(line.raw_outside_shift_hours)}；${line.overtime_approval_status || "无申请"}；确认计入：${hours(line.confirmed_overtime_hours)}`)}<br>${this.escape(`标准工时：${hours(line.standard_hours)}；导出实际出勤：${hours(line.actual_attendance_hours)}`)}<br>${this.escape(`有效请假：${leaveText}${excludedText ? `；周末不计入：${excludedText}` : ""}`)}<br>${this.escape(`加班：平日 ${hours(line.workday_overtime_hours)} / 休息日 ${hours(line.restday_overtime_hours)} / 节假日 ${hours(line.holiday_overtime_hours)}`)}${line.attendance_note ? `<br><strong>${this.escape(line.attendance_note)}</strong>` : ""}<br>${this.escape(`审批：${line.approval || line.overtime_approval || "--"}；来源：${line.source_file || "--"} / ${line.source_sheet || "--"} 第 ${line.source_row || "--"} 行`)}</small>`;
			return `<div class="hrms-attendance-exception-line ${resolved ? "is-resolved" : "is-pending"}" data-exception-card-record="${this.escape(recordId || "")}" ${locator}><strong>${this.escape(this.attendance_date_with_weekday(line.attendance_date))}</strong>　${this.escape(flags)}${facts}${comparison}${action}</div>`;
		}).join("");
	}

	render_attendance_daily_statuses(lines) {
		return (lines || []).map((line) => `<div class="hrms-attendance-exception-line"><strong>${this.escape(this.attendance_date_with_weekday(line.attendance_date))}</strong><br>${this.review_status_badge(line.review_status || "待审核")}</div>`).join("");
	}
	...
	result_trace(row, { includeApproval = true } = {}) {
		// The uploaded file path is preserved for audit, but is not useful on the
		// HR-facing result table.  Show only the sheet/row/business identifiers.
		const values = [row.source_sheet, row.source_row, row.source_id];
		if (includeApproval) values.push(row.approval_no);
	...
	render_missed_punch_summary(summary = {}) {
		const items = [
			["有效补卡笔数", summary.included_rows || 0, "neutral"],
	...
	render_processing_summary(title, items) {
		return `<section class="hrms-attendance-result-summary" aria-label="${this.escape(__(title))}"><strong>${this.escape(__(title))}</strong>${items.map(([label, value, tone]) => `<span class="is-${tone}"><small>${this.escape(__(label))}</small><b>${this.escape(value)}</b></span>`).join("")}</section>`;
	}
	...
	render_processing_results(rows = [], loading = false, meta = {}) {
		...
		const importErrorCount = Number(supportCheck?.import_error_count || 0);
		const importValidationMessage = meta.import_validation?.message || "";
		...
		const pageStart = Number(meta.page_start ?? ((this.processing_result_page - 1) * this.processing_result_page_size));
		...
		const resultTitle = isAttendanceDraft ? __("考勤初稿加工结果（完整汇总）") : isAppleTree ? __("苹果树加工结果（完整明细）") : isMissedPunch ? __("忘打卡加工结果（完整明细）") : isMonthlySupport ? __("{0}导入校验结果", [this.processing_source_label(this.selected_source_type)]) : __("加工结果");
		const resultDescription = isAttendanceDraft
			? __("按员工一行展示已可采用的钉钉明确数据。无异常记录会自动进入下游；待处理异常请在“异常处理”修改后再进入计算。")
			: isAppleTree
				? __("每条苹果树记录保留奖惩、审批与来源字段；结合奖惩日期、姓名和部门唯一匹配时自动补全工号。待处理异常保留在表中，不计入下游汇总。")
				: isMissedPunch
		...
		return `
			<div class="hrms-attendance-section">
				...
				${pagination}
				<div class="hrms-attendance-table-wrap"><table class="table table-bordered hrms-attendance-table"><thead><tr>${headers.map((header) => `<th>${this.escape(__(header))}</th>`).join("")}</tr></thead><tbody>${loading ? `<tr><td colspan="${headers.length}" class="text-muted">${this.escape(__("正在读取加工结果..."))}</td></tr>` : rows.length ? rows.map((row, index) => renderRow(row, pageStart + index)).join("") : `<tr><td colspan="${headers.length}" class="text-muted">${this.escape(pendingExceptionCount ? __("请先处理当前来源的异常。") : __("暂无加工结果；尚未完成加工时不会生成模拟数据。"))}</td></tr>`}</tbody></table></div>
				${pagination}
			</div>
		`;
	}
	...
	attendance_exception_event_text(row) {
		const events = this.processing_values(row).exception_events;
		if (!Array.isArray(events) || !events.length) return "--";
		const labels = {
			ATTENDANCE_HOURS_MISMATCH: "工时合计与标准工时不符",
			CLOCK_IN_MISSING: "上班缺卡", CLOCK_OUT_MISSING: "下班缺卡", LATE_MARKED: "迟到", EARLY_MARKED: "早退", ABSENCE_MARKED: "旷工标记", RESTDAY_CLOCKED_WITHOUT_OVERTIME: "休息日有打卡未计加班", RESTDAY_CLOCKED_WITHOUT_APPROVAL: "间接人员周末打卡缺加班单", SHIFT_MISSING: "班次缺失",
		};
		return events.map((event) => `${event.attendance_date || "--"} ${labels[event.code] || event.code}${event.count ? `×${event.count}` : ""}`).join("；");
	}
	...
	render_apple_tree_result_row(row, index) {
		...
		return `<tr><td>${this.escape(index + 1)}</td>${this.apple_tree_columns().map(([field]) => `<td class="${["项目", "备注"].includes(field) ? "hrms-attendance-long-cell" : ""}">${this.escape(values[field] ?? row[field] ?? "")}</td>`).join("")}<td class="hrms-attendance-long-cell">${this.escape(exception)}</td><td>${this.review_status_badge(row.review_status || "待审核")}</td><td>${this.escape(row.eligible_for_downstream ? "是" : "否")}</td><td class="hrms-attendance-trace" title="${this.escape(trace)}">${this.escape(trace)}</td><td><button class="btn btn-default btn-xs" data-edit-processing-record="${this.escape(row.record_id)}" data-edit-processing-source="apple_tree">${this.escape(this.processing_record_action_label(row))}</button></td></tr>`;
	}

	render_missed_punch_result_row(row, index) {
		const values = this.effective_missed_punch_values(row);
		const exceptionDetail = row.exception_detail || row.exception_message || "";
		const exception = row.exception_codes?.length ? `${this.exception_label_text(row)}${exceptionDetail ? `：${exceptionDetail}` : ""}` : "无";
		const trace = this.result_trace(row);
		const displayValue = (field) => field === "included" ? (values[field] ? "是" : "否") : (values[field] ?? row[field] ?? "");
		return `<tr><td>${this.escape(index + 1)}</td>${this.missed_punch_columns().map(([field]) => `<td class="${field === "reason" ? "hrms-attendance-long-cell" : ""}">${this.escape(displayValue(field))}</td>`).join("")}<td class="hrms-attendance-long-cell">${this.escape(exception)}</td><td>${this.review_status_badge(row.review_status || "待审核")}</td><td class="hrms-attendance-trace" title="${this.escape(trace)}">${this.escape(trace)}</td><td><button class="btn btn-default btn-xs" data-edit-processing-record="${this.escape(row.record_id)}" data-edit-processing-source="missing_card">${this.escape(this.processing_record_action_label(row))}</button></td></tr>`;
	}

	render_monthly_support_result_row(row, index, columns) {
		const values = this.processing_values(row);
		const exception = row.exception_codes?.length ? `${this.exception_label_text(row)}${row.exception_message ? `：${row.exception_message}` : ""}` : "无";
		const trace = this.result_trace(row, { includeApproval: false });
		const valueFor = (field) => field === "special_hours_days" ? this.format_special_hours_days(values[field]) : (values[field] ?? row[field] ?? "");
		const importStatus = row.exception_codes?.length ? __("导入错误") : __("导入通过");
		return `<tr><td>${this.escape(index + 1)}</td>${columns.map(([field]) => `<td class="${field === "special_hours_days" ? "hrms-attendance-long-cell" : ""}">${this.escape(valueFor(field))}</td>`).join("")}<td class="hrms-attendance-long-cell">${this.escape(exception)}</td><td>${this.status_badge(importStatus)}</td><td class="hrms-attendance-trace" title="${this.escape(trace)}">${this.escape(trace)}</td><td><button class="btn btn-default btn-xs" data-edit-processing-record="${this.escape(row.record_id)}" data-edit-processing-source="${this.escape(this.selected_source_type)}">${this.escape(this.processing_record_action_label(row))}</button></td></tr>`;
	}
	...
	bind_processing_result_events() {
		...
		body.querySelectorAll("[data-processing-result-page]").forEach((button) => button.addEventListener("click", () => {
			const page = Number(button.dataset.processingResultPage);
			if (!Number.isInteger(page) || page < 1 || page === this.processing_result_page) return;
			this.processing_result_page = page;
			this.load_processing_results();
		}));
			...
			primary_action: (values) => {
				...
				this.call_processing_api(
					"bulk_update_processing_records",
					{
						...
						employee_code: this.exception_employee_code_filter,
						employee_name: this.exception_employee_name_filter,
					},
					{
						freeze: true,
	...
	download_processing_file(fileUrl, fileName = "") {
		// An anchor click is not blocked as an asynchronous popup and works for
		// authenticated /private/files links as well as regular download URLs.
	...
	parse_attendance_time_minutes(value) {
	...
	overtime_wheel_column_markup(part, max) {
		const values = ["", ...Array.from({ length: max }, (_, index) => String(index).padStart(2, "0"))];
		return `<div class="hrms-overtime-wheel-column" data-overtime-wheel-column="${part}" role="listbox" aria-label="${part === "hour" ? this.escape(__("小时")) : this.escape(__("分钟"))}"><div class="hrms-overtime-wheel-spacer" aria-hidden="true"></div>${values.map((value, index) => `<div class="hrms-overtime-wheel-option" data-overtime-wheel-option data-wheel-index="${index}" data-wheel-value="${value}" role="option" tabindex="0">${value || "--"}</div>`).join("")}<div class="hrms-overtime-wheel-spacer" aria-hidden="true"></div></div>`;
	}

	overtime_wheel_group_markup(fieldname, label) {
		return `<div class="hrms-overtime-wheel-group" data-overtime-wheel-group="${fieldname}"><div class="hrms-overtime-wheel-label">${this.escape(__(label))}</div><input type="text" class="form-control hrms-overtime-wheel-input" data-overtime-wheel-input="${fieldname}" placeholder="HH:mm" autocomplete="off" inputmode="text" aria-label="${this.escape(__(label))}"><div class="hrms-overtime-wheel" data-overtime-wheel="${fieldname}"><div class="hrms-overtime-wheel-selection" aria-hidden="true"></div>${this.overtime_wheel_column_markup("hour", 24)}<span class="hrms-overtime-wheel-separator" aria-hidden="true">:</span>${this.overtime_wheel_column_markup("minute", 60)}</div><button type="button" class="btn btn-link btn-xs hrms-overtime-wheel-clear" data-overtime-clear="${fieldname}">${this.escape(__("清空"))}</button></div>`;
	}

	overtime_wheel_picker_markup(sourcePunchButtons = "") {
		return `<div class="hrms-overtime-wheel-picker" data-overtime-wheel-picker>${this.overtime_wheel_group_markup("start", "开始时间（可选）")}${this.overtime_wheel_group_markup("end", "结束时间（可选）")}</div>${sourcePunchButtons ? `<div class="small text-muted hrms-overtime-wheel-source-actions">${sourcePunchButtons}</div>` : ""}`;
	}

	overtime_wheel_scroll_time(dialog, fieldname) {
		const root = dialog.$wrapper?.[0]?.querySelector(`[data-overtime-wheel="${fieldname}"]`);
		if (!root) return "";
		const valueFor = (part) => {
			const column = root.querySelector(`[data-overtime-wheel-column="${part}"]`);
			if (!column) return "";
			const index = Math.max(0, Math.min(Math.round(column.scrollTop / 44), column.querySelectorAll("[data-wheel-value]").length - 1));
			return column.querySelectorAll("[data-wheel-value]")[index]?.dataset.wheelValue || "";
		};
		const hour = valueFor("hour");
		const minute = valueFor("minute");
		return hour && minute ? `${hour}:${minute}:00` : "";
	}

	overtime_wheel_time(dialog, fieldname) {
		const root = dialog.$wrapper?.[0]?.querySelector(`[data-overtime-wheel="${fieldname}"]`);
		const input = root?.parentElement?.querySelector(`[data-overtime-wheel-input="${fieldname}"]`);
		const typed = String(input?.value || "").trim();
		return typed || this.overtime_wheel_scroll_time(dialog, fieldname);
	}

	sync_overtime_wheel_input(dialog, fieldname) {
		const root = dialog.$wrapper?.[0]?.querySelector(`[data-overtime-wheel="${fieldname}"]`);
		const input = root?.parentElement?.querySelector(`[data-overtime-wheel-input="${fieldname}"]`);
		if (!input) return;
		const value = this.overtime_wheel_scroll_time(dialog, fieldname);
		input.value = value ? value.slice(0, 5) : "";
		input.classList.remove("is-invalid");
	}

	set_overtime_wheel_time(dialog, fieldname, value) {
		const root = dialog.$wrapper?.[0]?.querySelector(`[data-overtime-wheel="${fieldname}"]`);
		if (!root) return;
		const normalized = this.attendance_time_control_value(value);
		const [hour = "", minute = ""] = normalized.split(":");
		const input = root.parentElement?.querySelector(`[data-overtime-wheel-input="${fieldname}"]`);
		if (input) input.value = normalized ? `${hour}:${minute}` : "";
		[["hour", hour], ["minute", minute]].forEach(([part, selected]) => {
			const column = root.querySelector(`[data-overtime-wheel-column="${part}"]`);
			const option = column && [...column.querySelectorAll("[data-wheel-value]")].find((item) => item.dataset.wheelValue === selected);
			if (column && option) column.scrollTop = Number(option.dataset.wheelIndex) * 44;
		});
	}
	...
	open_attendance_daily_row_editor(recordId, sourceRow, restdayOvertimeCorrection = false, sourceContext = {}) {
		const scrollPosition = this.capture_exception_scroll_position(recordId, sourceRow, sourceContext);
		this.call_processing_api(
			"get_processing_record",
			{ company: this.company, attendance_month: this.attendance_month, source_type: "attendance_draft", record_id: recordId },
			{
				on_success: (record) => {
					...
					const lateCorrection = (dailyRow.exception_codes || []).includes("LATE_MARKED");
					const editableFields = restdayOvertimeCorrection
						? (dailyRow.editable_fields || []).filter((field) => field.label === "休息日加班（小时）")
						: lateCorrection
							? (dailyRow.editable_fields || []).filter((field) => field.late_editor)
					...
					const restdayFacts = this.escape(__("填写时核对：班次：{0}；{1}；实际打卡：{2} → {3}；班次外原始时长：{4} 小时；{5}；确认计入：{6} 小时；标准工时：{7} 小时；导出实际出勤：{8} 小时。不足 30 分钟不计，每满 30 分钟计 0.5 小时。", [
						dailyRow.shift || "--", dailyRow.is_weekend ? "周末，只核对时长" : dailyRow.date_type || "--",
						dailyRow.clock_in || "--", dailyRow.clock_out || "--", dailyRow.raw_outside_shift_hours ?? "--",
						dailyRow.overtime_approval_status || "无申请", dailyRow.confirmed_overtime_hours ?? 0,
						dailyRow.standard_hours ?? 0, dailyRow.actual_attendance_hours ?? 0,
					]));
					let dialog;
					const fields = restdayOvertimeCorrection
						? [
							...
							{
								fieldtype: "HTML",
								fieldname: "restday_overtime_wheels",
								options: this.overtime_wheel_picker_markup(sourcePunchButtons),
							},
							{
								fieldtype: "Float",
						...
						: editableFields.map((field, index) => ({
							fieldtype: "Data",
							fieldname: `daily_value_${index}`,
							label: field.label,
							default: field.value ?? "",
						}));
					if (lateCorrection && !restdayOvertimeCorrection) {
						fields.unshift({
							fieldtype: "Data", fieldname: "attendance_date_display", label: __("日期"),
							default: this.attendance_date_with_weekday(dailyRow.attendance_date), read_only: 1,
						});
					}
					fields.unshift({
						fieldtype: "HTML",
						options: restdayOvertimeCorrection
							? `<div class="hrms-attendance-dialog-note"><strong>${this.escape(this.attendance_date_with_weekday(dailyRow.attendance_date))}</strong><br>${restdayFacts}</div>`
							: `<div class="hrms-attendance-dialog-note"><strong>${this.escape(__("钉钉原始行：{0} / 第 {1} 行", [this.attendance_date_with_weekday(dailyRow.attendance_date), dailyRow.source_row]))}</strong><br>${this.escape(__("在这里修改的值会保留原值、原因和操作人，并重新汇总该员工当月数据；不会修改上传的 Excel 原件。"))}</div>`,
					});
					fields.push(
					...
					dialog = new frappe.ui.Dialog({
						title: __(restdayOvertimeCorrection ? "补录休息日加班：{0} {1}" : "修改钉钉每日数据：{0} {1}", [record.employee_code || "", record.employee_name || ""]),
						fields,
						primary_action_label: __("保存本日修改并重新汇总"),
						primary_action: (values) => {
							const changes = {};
							let overtimeStartValue = "";
							let overtimeEndValue = "";
							if (restdayOvertimeCorrection) {
								overtimeStartValue = this.overtime_wheel_time(dialog, "start");
								overtimeEndValue = this.overtime_wheel_time(dialog, "end");
								const startMinutes = overtimeStartValue ? this.parse_attendance_time_minutes(overtimeStartValue) : null;
								const endMinutes = overtimeEndValue ? this.parse_attendance_time_minutes(overtimeEndValue) : null;
								if ((overtimeStartValue && startMinutes === null) || (overtimeEndValue && endMinutes === null)) {
									frappe.msgprint(__("开始时间和结束时间如有填写，必须是有效时间；两项也可以都不填写。"));
									return;
								}
								...
								editableFields.forEach((field, index) => {
									const next = values[`daily_value_${index}`] ?? "";
									if (String(next) !== String(field.value ?? "")) changes[field.fieldname] = next;
								});
							}
							if (!Object.keys(changes).length) {
								frappe.msgprint(__("未修改任何字段。"));
								return;
							}
							this.call_processing_api(
								"update_attendance_draft_daily_row",
									{ company: this.company, attendance_month: this.attendance_month, record_id: recordId, source_row: dailyRow.source_row, source_file: dailyRow.source_file, source_sheet: dailyRow.source_sheet, attendance_date: dailyRow.attendance_date, changes, review_status: values.review_status, reason: values.reason, overtime_start_time: overtimeStartValue, overtime_end_time: overtimeEndValue },
								{
									freeze: true,
									...
									on_success: (data) => {
										dialog.hide();
										frappe.show_alert({
											message: __(data.review_status === "待审核" ? "已保存本日修改并重新校验；本日或其他日期仍有异常，请继续核对。" : "已保存本日修改，并已更新该员工的下游汇总。"),
											indicator: data.review_status === "待审核" ? "orange" : "green",
										});
										this.load_processing_exceptions({ preserveScroll: scrollPosition });
									},
								},
							);
						},
					...
					dialog.show();
					if (restdayOvertimeCorrection) {
						dialog.$wrapper.find("[data-overtime-wheel-option]").on("click", (event) => {
							const option = event.currentTarget;
							const column = option.parentElement;
							column.scrollTo({ top: Number(option.dataset.wheelIndex) * 44, behavior: "smooth" });
						});
						dialog.$wrapper.find("[data-overtime-wheel-column]").on("scroll", (event) => {
							const column = event.currentTarget;
							const wheel = column.closest("[data-overtime-wheel]");
							if (wheel?.dataset.overtimeWheel) this.sync_overtime_wheel_input(dialog, wheel.dataset.overtimeWheel);
						});
						dialog.$wrapper.find("[data-overtime-wheel-input]").on("input change blur", (event) => {
							const input = event.currentTarget;
							const fieldname = input.dataset.overtimeWheelInput;
							const typed = String(input.value || "").trim();
							const normalized = this.attendance_time_control_value(typed);
							input.classList.toggle("is-invalid", Boolean(typed && !normalized));
							if (!typed || normalized) this.set_overtime_wheel_time(dialog, fieldname, normalized);
						});
						dialog.$wrapper.find("[data-overtime-clear]").on("click", (event) => {
							this.set_overtime_wheel_time(dialog, event.currentTarget.dataset.overtimeClear, "");
						});
						dialog.$wrapper.find("[data-use-source-clock-in]").on("click", () => {
							this.set_overtime_wheel_time(dialog, "start", sourceClockIn);
						});
						dialog.$wrapper.find("[data-use-source-clock-out]").on("click", () => {
							this.set_overtime_wheel_time(dialog, "end", sourceClockOut);
						});
						}
				},
				on_error: (message) => frappe.msgprint(message),
	...
	confirm_attendance_daily_no_overtime(recordId, sourceRow, attendanceDate = "", sourceContext = {}) {
		const scrollPosition = this.capture_exception_scroll_position(recordId, sourceRow, { ...sourceContext, attendanceDate });
		const dialog = new frappe.ui.Dialog({
			title: __("确认本日不计加班：{0}", [attendanceDate || "--"]),
			fields: [
				{ fieldtype: "HTML", options: `<div class="hrms-attendance-dialog-note">${this.escape(__("本次只处理 {0} 这一天；同一员工的其他异常日期会继续保留在待处理列表中。", [attendanceDate || "--"]))}</div>` },
				{ fieldtype: "Small Text", fieldname: "reason", label: __("确认原因"), reqd: 1, default: __("已核对该休息日打卡，本日不计入休息日加班。") },
			],
			primary_action_label: __("确认本日不计加班并留痕"),
			primary_action: (values) => {
				this.call_processing_api(
					...
					{
						freeze: true,
						freeze_message: __("正在记录本日处理决定..."),
						on_success: (data) => {
							dialog.hide();
							frappe.show_alert({
								message: __(data.review_status === "待审核" ? "已处理本日；该员工其他异常日期仍待处理。" : "已处理本日，该员工已无待处理异常日期。"),
								indicator: data.review_status === "待审核" ? "orange" : "green",
							});
							this.load_processing_exceptions({ preserveScroll: scrollPosition });
						},
	...
	show_processing_record_dialog(record) {
		...
		if (dailyLines.some((line) => (line.exception_codes || []).some((code) => ["RESTDAY_CLOCKED_WITHOUT_OVERTIME", "RESTDAY_CLOCKED_WITHOUT_APPROVAL"].includes(code)))) {
			const dailyDialog = new frappe.ui.Dialog({
				title: __("按日期处理：{0} {1}", [record.employee_code || "", record.employee_name || ""]),
				fields: [{
					fieldtype: "HTML",
					options: `<div class="hrms-attendance-dialog-note">${this.escape(__("同一员工有多个异常日期时，每天分别处理和留痕；未处理的日期不会被一起通过。"))}<div class="hrms-attendance-exception-lines">${this.render_attendance_exception_lines(dailyLines, record.record_id || record.name || "")}</div></div>`,
				}],
			});
			dailyDialog.show();
	...
	confirm_source_result(sourceType) {
		this.call_processing_api(
			...
			{ company: this.company, attendance_month: this.attendance_month, source_type: sourceType },
			{
				...
				on_success: (data) => {
					frappe.show_alert({ message: __("本类结果已确认；{0} 条未处理异常已排除在下游计算之外。", [data.pending_review_rows ?? data.pending_rows ?? 0]), indicator: "green" });
					if (this.active_view !== "monthly-final") this.set_view("monthly-final");
					else this.load_monthly_final();
				},
				on_error: (message) => frappe.msgprint(message),
	...
	load_processing_exceptions(options = {}) {
		...
		const args = {
			...
			employee_code: this.exception_employee_code_filter,
			employee_name: this.exception_employee_name_filter,
			department: this.exception_department_filter,
			exception_code: this.exception_code_filter,
			processing_status: this.exception_processing_status_filter,
			sort_field: this.exception_sort_field,
			sort_order: this.exception_sort_order,
		};
		if (this.exception_source_filter) args.source_type = this.exception_source_filter;
		if (preserveScroll?.recordId) args.focus_record_id = preserveScroll.recordId;
		const snapshotKey = JSON.stringify({
			company: args.company, attendance_month: args.attendance_month, source_type: args.source_type || "",
			employee_code: args.employee_code, employee_name: args.employee_name, department: args.department,
			exception_code: args.exception_code, processing_status: args.processing_status,
			sort_field: args.sort_field, sort_order: args.sort_order,
		});
		const snapshot = options.reuseSnapshot && this.exception_query_snapshot?.key === snapshotKey
			? this.exception_query_snapshot : null;
		if (snapshot) {
		...
		this.call_processing_api(
			...
			args,
			{
				on_success: (data) => {
					...
					if (this.exception_page > totalPages) {
						this.exception_page = totalPages;
						this.load_processing_exceptions(options);
						return;
					}
				...
				on_error: (message) => {
					if (requestId !== this.exception_query_request_id || this.active_view !== "exceptions") return;
					body.innerHTML = this.render_processing_exceptions([], false, message);
					this.bind_processing_exception_events();
					this.restore_exception_scroll_position(preserveScroll);
				},
			},
	...
	render_exception_employee_controls() {
		return `<div class="hrms-attendance-exception-query"><small>${this.escape(__("可在固定表头中搜索、筛选和排序"))}</small><button type="button" class="btn btn-link btn-sm" data-clear-exception-query>${this.escape(__("清除全部筛选"))}</button></div>`;
	}

	export_processing_exceptions() {
		this.call_processing_api("export_processing_exceptions", {
			company: this.company,
			attendance_month: this.attendance_month,
			source_type: this.exception_source_filter,
			employee_code: this.exception_employee_code_filter,
			employee_name: this.exception_employee_name_filter,
			department: this.exception_department_filter,
			exception_code: this.exception_code_filter,
			...
			freeze: true,
			freeze_message: __("正在生成当前筛选的异常明细..."),
			on_success: (data) => {
				if (!data.file_url) return frappe.msgprint(__("未能生成异常导出文件。"));
				this.download_processing_file(data.file_url, data.file_name);
				frappe.show_alert({ message: __("已导出 {0} 条异常日期明细。", [data.exception_line_count || 0]), indicator: "green" });
			},
			on_error: (message) => frappe.msgprint(message),
		});
	}
	...
	exception_type_options() {
		return [
			["CLOCK_IN_MISSING", "上班缺卡"], ["CLOCK_OUT_MISSING", "下班缺卡"], ["LATE_MARKED", "迟到"],
			["EARLY_MARKED", "早退"], ["ABSENCE_MARKED", "旷工"], ["ATTENDANCE_HOURS_MISMATCH", "工时不一致"],
			["RESTDAY_CLOCKED_WITHOUT_OVERTIME", "休息日有打卡未计加班"],
			["RESTDAY_CLOCKED_WITHOUT_APPROVAL", "间接人员周末打卡缺加班单"],
			["WORKDAY_OUTSIDE_SHIFT_UNAPPROVED", "钉钉加班为0：缺加班单"],
			["WORKDAY_OVERTIME_APPROVAL_NOT_APPLIED", "钉钉加班为0：审批未生效"],
			["WORKDAY_OVERTIME_DINGTALK_ANOMALY", "钉钉加班为0：免审批计算异常"],
			["SHIFT_SCHEDULE_REVIEW_REQUIRED", "班次计划待复核"],
		];
	}

	render_exception_sort_label(label, field) {
		const active = this.exception_sort_field === field;
		const indicator = active ? (this.exception_sort_order === "desc" ? "↓" : "↑") : "↕";
		return `<span class="hrms-attendance-exception-heading"><span>${this.escape(__(label))}</span><button type="button" data-exception-sort-by="${this.escape(field)}" aria-label="${this.escape(__("按{0}排序", [label]))}">${indicator}</button></span>`;
	}

	render_exception_table_header(canBulkProcess, currentPending) {
		const sourceOptions = (this.exception_sources || []).map((source) => `<option value="${this.escape(source.key)}" ${this.exception_source_filter === source.key ? "selected" : ""}>${this.escape(__(source.label))}</option>`).join("");
		const exceptionOptions = this.exception_type_options().map(([code, label]) => `<option value="${this.escape(code)}" ${this.exception_code_filter === code ? "selected" : ""}>${this.escape(__(label))}</option>`).join("");
		const departments = [...new Set([...(this.exception_department_options || []), this.exception_department_filter].filter(Boolean))].sort((left, right) => left.localeCompare(right, "zh-CN"));
		const departmentOptions = departments.map((department) => `<option value="${this.escape(department)}" ${this.exception_department_filter === department ? "selected" : ""}>${this.escape(department)}</option>`).join("");
		return `<thead><tr>
			<th><input type="checkbox" data-select-exception-all ${canBulkProcess && currentPending ? "" : "disabled"} ${this.select_all_filtered_exceptions ? "checked" : ""} title="${this.escape(__("全选当前筛选来源的全部待处理异常"))}"></th>
			<th>${this.render_exception_sort_label("姓名", "employee_name")}<input type="text" class="form-control input-sm" data-exception-employee-name-filter value="${this.escape(this.exception_employee_name_filter)}" placeholder="${this.escape(__("搜索"))}"></th>
			<th>${this.render_exception_sort_label("工号", "employee_code")}<input type="text" class="form-control input-sm" data-exception-employee-code-filter value="${this.escape(this.exception_employee_code_filter)}" placeholder="${this.escape(__("搜索"))}"></th>
			<th>${this.render_exception_sort_label("部门", "department")}<select class="form-control input-sm" data-exception-department-filter><option value="">${this.escape(__("全部"))}</option>${departmentOptions}</select></th>
			<th>${this.render_exception_sort_label("来源", "source_type")}<select class="form-control input-sm" data-exception-source-filter><option value="">${this.escape(__("全部"))}</option>${sourceOptions}</select></th>
			<th>${this.render_exception_sort_label("异常类型、日期及原因", "exception_code")}<select class="form-control input-sm" data-exception-code-filter><option value="">${this.escape(__("全部异常"))}</option>${exceptionOptions}</select></th>
			<th>${this.render_exception_sort_label("处理状态", "review_status")}<select class="form-control input-sm" data-exception-processing-status-filter><option value="">${this.escape(__("全部"))}</option><option value="pending" ${this.exception_processing_status_filter === "pending" ? "selected" : ""}>${this.escape(__("待处理"))}</option><option value="processed" ${this.exception_processing_status_filter === "processed" ? "selected" : ""}>${this.escape(__("已处理"))}</option></select></th>
			<th><span class="hrms-attendance-exception-heading"><span>${this.escape(__("操作"))}</span></span></th>
		</tr></thead>`;
	}
	...
	render_processing_exceptions(rows = [], loading = false, error = "", summary = {}) {
		...
		const policyNotice = rows.some((row) => row.attendance_policy_stale) ? `<div class="hrms-attendance-api-notice"><strong>本月部分数据尚未按当前映射识别异常</strong><span>钉钉原始加班不会重算；只会重新分类“加班为0且明显班后打卡”的异常。</span></div>` : "";
	...
	bind_processing_exception_events() {
		...
		const applyExceptionQuery = () => {
			...
			this.exception_employee_name_filter = body.querySelector("[data-exception-employee-name-filter]")?.value.trim() || "";
			this.exception_department_filter = body.querySelector("[data-exception-department-filter]")?.value.trim() || "";
			const sourceFilter = body.querySelector("[data-exception-source-filter]");
			this.exception_source_filter = sourceFilter ? sourceFilter.value : (this.exception_source_filter || "");
			this.exception_code_filter = body.querySelector("[data-exception-code-filter]")?.value || "";
			this.exception_processing_status_filter = body.querySelector("[data-exception-processing-status-filter]")?.value || "";
			this.exception_page = 1;
			this.selected_exception_record_ids.clear();
			this.select_all_filtered_exceptions = false;
			this.load_processing_exceptions();
		};
		body.querySelector("[data-apply-exception-query]")?.addEventListener("click", applyExceptionQuery);
		body.querySelector("[data-clear-exception-query]")?.addEventListener("click", () => {
			this.exception_employee_code_filter = "";
			this.exception_employee_name_filter = "";
			this.exception_department_filter = "";
			this.exception_code_filter = "";
		...
		body.querySelectorAll("[data-exception-sort-by]").forEach((button) => button.addEventListener("click", () => {
			const field = button.dataset.exceptionSortBy;
			this.exception_sort_order = this.exception_sort_field === field && this.exception_sort_order === "asc" ? "desc" : "asc";
			this.exception_sort_field = field;
			applyExceptionQuery();
		}));
		body.querySelectorAll("[data-exception-source]").forEach((button) => button.addEventListener("click", () => { this.exception_source_filter = button.dataset.exceptionSource; this.exception_page = 1; this.selected_exception_record_ids.clear(); this.load_processing_exceptions(); }));
		body.querySelectorAll("[data-exception-page]").forEach((button) => button.addEventListener("click", () => {
			const page = Number(button.dataset.exceptionPage);
			if (!Number.isInteger(page) || page < 1 || page === this.exception_page) return;
			this.exception_page = page;
			this.selected_exception_record_ids.clear();
			this.load_processing_exceptions({ reuseSnapshot: true });
		}));
	...
	load_processing_ledger(kind) {
		...
		if (!this.ensure_company()) return;
		const args = { company: this.company, attendance_month: this.attendance_month };
		// The ledger is ordered newest-first. Bound the initial payload so opening
		// the page does not parse years of JSON history before showing this month.
		if (kind === "adjustments") args.page_length = 500;
		this.call_processing_api(method, args, {
			on_success: (data) => {
				if (requestId !== this.ledger_query_request_id || this.active_view !== expectedView) return;
				body.innerHTML = this.render_processing_ledger(kind, data.rows || data.items || [], false, "", data);
				this.bind_processing_ledger_events(kind);
			},
			on_error: (message) => {
				if (requestId !== this.ledger_query_request_id || this.active_view !== expectedView) return;
				body.innerHTML = this.render_processing_ledger(kind, [], false, message);
				this.bind_processing_ledger_events(kind);
			},
		});
	}
	...
	render_processing_ledger(kind, rows = [], loading = false, error = "", meta = {}) {
		...
		const boundedNotice = !isBatch && meta.has_more
	...
	bind_processing_ledger_events(kind) {
		...
		this.body()?.querySelector("[data-reset-attendance-month]")?.addEventListener("click", () => this.open_reset_attendance_month_dialog());
	}

	open_reset_attendance_month_dialog() {
		if (!this.ensure_company()) return;
		const month = this.attendance_month;
		const dialog = new frappe.ui.Dialog({
			title: __("清空 {0} 考勤数据", [this.format_attendance_month(month)]),
			fields: [
				{ fieldname: "warning", fieldtype: "HTML", options: `<div class="alert alert-warning">${this.escape(__("将删除本月六类考勤来源的加工记录、异常处理、人工调整和终稿引用。原始上传文件会保留，但本月需要重新导入并加工。"))}</div>` },
				{ fieldname: "confirm_month", fieldtype: "Data", label: __("请输入 {0} 确认", [month]), reqd: 1 },
			],
			primary_action_label: __("彻底清空"),
			primary_action: (values) => this.call_processing_api("reset_attendance_month", {
				company: this.company,
				attendance_month: month,
				confirm_month: values.confirm_month,
			}, {
				freeze: true,
				freeze_message: __("正在清空本月考勤数据..."),
				on_success: (data) => {
					dialog.hide();
					this.processing_batch = null;
					this.daily_attendance_data = null;
					this.daily_attendance_date = "";
					frappe.show_alert({ message: data.notice || __("本月数据已清空。"), indicator: "green" });
					this.load_processing_ledger("batches");
				},
				on_error: (message) => frappe.msgprint(message),
			}),
		});
	...
	load_processing_configuration() {
		if (this.active_view === "department-mapping") return this.load_department_mappings();
	...
	refresh_attendance_rule_view() {
		if (this.active_view === "processing-rules") return this.load_complete_attendance_rules();
		return this.load_attendance_shift_rules();
	}

	load_complete_attendance_rules() {
		const body = this.body();
		body.innerHTML = this.render_complete_attendance_rules({}, true);
		if (!this.ensure_company()) return;
		this.call_processing_api("get_complete_attendance_rules", { company: this.company }, {
			on_success: (data) => {
				body.innerHTML = this.render_complete_attendance_rules(data || {}, false);
				this.bind_complete_attendance_rule_events(data || {});
			},
			on_error: (message) => {
				body.innerHTML = this.render_complete_attendance_rules({}, false, message);
				this.bind_complete_attendance_rule_events({});
			},
		});
	}
	...
	render_complete_attendance_rules(data = {}, loading = false, error = "") {
		...
		const fallbackNotice = data.using_builtin_fallback
			? `<div class="hrms-attendance-api-notice"><strong>${this.escape(__("尚未发布公司规则"))}</strong><span>${this.escape(__("当前仅使用内置兼容规则。正式启用前须完整导入并逐项复核。"))}</span></div>`
			: "";
		...
		<div class="hrms-attendance-kpi-grid hrms-attendance-rule-kpis"><div class="hrms-attendance-kpi"><strong>${this.escape(shiftRules.length)}</strong><span>${this.escape(__("公司班次规则"))}</span><small>${this.escape(__("启用 {0} 条", [shiftRules.filter((row) => row.enabled).length]))}</small></div><div class="hrms-attendance-kpi"><strong>${this.escape(data.shift_rule_version || "--")}</strong><span>${this.escape(__("当前规则版本"))}</span><small>${this.escape(__("结果保留版本快照"))}</small></div><div class="hrms-attendance-kpi"><strong>${this.escape(__("公司级"))}</strong><span>${this.escape(__("规则适用范围"))}</span><small>${this.escape(__("导入不按处理月份分批；生效日仅用于版本边界"))}</small></div></div>
		...
		<section class="hrms-attendance-rule-block"><div class="hrms-attendance-list-head"><div><h4>${this.escape(__("二、班次匹配与异常分类规则"))}</h4><small>${this.escape(__("用于确定是否需要审批、明显班后时段和异常类型；不重新计算钉钉已给出的工作日加班。"))}</small></div></div><div class="hrms-attendance-rule-browser" data-rule-browser>${loading ? this.escape(__("正在读取班别...")) : ""}</div></section>
	...
	render_attendance_rule_browser(data = {}) {
		...
		const sourceColumns = new Map((data.source_column_mapping || []).map((item) => [item.fieldname, item]));
		let selectedSourceRaw = [];
		try { selectedSourceRaw = JSON.parse(selected?.source_payload_json || "[]"); } catch (_error) { selectedSourceRaw = []; }
		const field = (label, value, sourceField = "") => {
			const source = sourceColumns.get(sourceField);
			const rawValue = source ? selectedSourceRaw[source.column.charCodeAt(0) - 65] : null;
			const sourceNote = source && selected?.source_row
				? `<small>${this.escape(`Excel ${source.column}${selected.source_row}：${rawValue === 0 ? "0" : rawValue || "--"}；${source.application}`)}</small>`
				: "";
			return '<div class="hrms-attendance-rule-field"><span>' + this.escape(__(label)) + '</span><strong>' + this.escape(value === 0 ? "0" : value || "--") + '</strong>' + sourceNote + '</div>';
		};
		if (!this.attendance_rule_group) {
			...
			const rawFixedHours = selectedSourceRaw[11];
			const fixedHoursMismatch = rawFixedHours !== "" && rawFixedHours != null
				&& Number.isFinite(Number(rawFixedHours))
				&& Number(rawFixedHours) !== Number(selected.weekday_overtime_hours || 0);
			...
			const details = [
				...
				["基本班次", selected.basic_time, "basic_time"], ["基本工时", selected.basic_hours == null ? "" : selected.basic_hours + " 小时", "basic_hours"],
				["休息扣除", selected.meal_deduction_rule, "meal_deduction_rule"], ["平日特殊工时时段", selected.special_workday_time, "extended_shift_rule"],
				["平日加班时段", selected.weekday_overtime_time, "weekday_overtime_time"], ["平日固定加班", selected.weekday_overtime_hours == null ? "" : selected.weekday_overtime_hours + " 小时", "weekday_overtime_hours"],
				["平日加班是否需申请", selected.weekday_overtime_mode, "weekday_overtime_hours"], ["固定加班后续是否需申请", selected.extended_overtime_mode, "extended_shift_rule"],
				["加班审批时间校验", selected.overtime_approval_time_mode], ["审批结束后再次申请阈值", `${selected.overtime_approval_reapply_minutes ?? 30} 分钟`],
				["后续延班原文", selected.extended_shift_rule, "extended_shift_rule"], ["周末加班时段（来源记录）", selected.weekend_overtime_time, "weekend_overtime_time"],
				["周末加班是否需申请", selected.weekend_overtime_mode, "weekend_overtime_mode"], ["节日加班是否需申请（不自动计薪）", selected.holiday_overtime_mode, "holiday_overtime_mode"],
				["班后开始加班", selected.overtime_begin_time, "overtime_begin_time"], ["是否隔夜", selected.overnight ? "是" : "否", "overnight"],
				["小夜班", selected.small_night_rule, "small_night_rule"], ["大夜班", selected.large_night_rule, "large_night_rule"],
				["可取上班卡", selected.punch_in_range, "punch_in_range"], ["可取下班卡", selected.punch_out_range, "punch_out_range"], ["生效日期", selected.effective_from],
				["系统班次", selected.shift_type], ["建议岗位", selected.suggested_positions, "suggested_positions"],
				["备注", selected.remarks, "remarks"], ["来源文件", selected.source_file],
			];
	...
	load_attendance_shift_rules() {
		if (this.active_view === "schedule-management") return this.load_complete_attendance_rules();
		const body = this.body();
		body.innerHTML = this.render_attendance_shift_rules({}, true);
		if (!this.ensure_company()) return;
		this.call_processing_api("list_attendance_shift_rules", { company: this.company }, {
			on_success: (data) => { body.innerHTML = this.render_attendance_shift_rules(data || {}, false); this.bind_attendance_shift_rule_events(data?.items || []); },
			on_error: (message) => { body.innerHTML = this.render_attendance_shift_rules({}, false, message); this.bind_attendance_shift_rule_events([]); },
		});
	}

	render_attendance_shift_rules(data = {}, loading = false, error = "") {
		const rows = data.items || [];
		const fallbackNotice = data.using_builtin_fallback
			? `<div class="hrms-attendance-api-notice"><strong>${this.escape(__("尚未导入公司排班规则"))}</strong><span>${this.escape(__("当前考勤继续使用代码内置的 V3.0 兜底规则。首次导入排班表后，后续加工会读取本页启用的结构化规则。"))}</span></div>`
			: "";
		const renderRow = (row) => `<tr><td>${row.enabled ? this.escape(__("启用")) : this.escape(__("停用"))}</td><td><strong>${this.escape(row.rule_name || "--")}</strong><br><small>${this.escape(row.rule_code || "")}</small></td><td>${this.escape(row.match_tokens || "--")}</td><td>${this.escape(row.basic_time || "--")}<br><small>${this.escape(__("基本工时 {0} 小时", [row.basic_hours ?? "--"]))}</small></td><td>${this.escape(row.weekday_overtime_mode || "--")}<br><small>${this.escape(__("{0} 小时；{1}", [row.weekday_overtime_hours ?? 0, row.weekday_overtime_time || "未配置时段"]))}</small></td><td>${this.escape(row.weekend_overtime_mode || "--")}<br><small>${this.escape(row.holiday_overtime_mode || "")}</small></td><td>${this.escape(row.punch_in_range || "--")}<br><small>${this.escape(row.punch_out_range || "--")}</small></td><td class="hrms-attendance-long-cell">${this.escape(row.small_night_rule || "无")}<br>${this.escape(row.large_night_rule || "")}</td><td>${this.escape(row.source_version || "手动")}<br><small>${this.escape(row.source_file || "--")}</small></td><td><button class="btn btn-default btn-xs" data-edit-shift-rule="${this.escape(row.name || "")}">${this.escape(__("编辑"))}</button></td></tr>`;
		return `<div class="hrms-attendance-section"><div class="hrms-attendance-list-head"><div><h3>${this.escape(__("排班管理"))}</h3><small>${this.escape(__("排班表只在预览确认后导入。启用规则直接参与班次匹配、加班来源、可取卡区间及可结构化的夜班津贴判断；修改后历史考勤需重新校验。"))}</small></div><div><button class="btn btn-default btn-sm" data-import-shift-rules>${this.escape(__(rows.length ? "重新导入排班表" : "首次导入排班表"))}</button> <button class="btn btn-primary btn-sm" data-add-shift-rule>${this.escape(__("新增规则"))}</button></div></div>${fallbackNotice}${error ? `<div class="hrms-attendance-api-notice"><strong>${this.escape(__("接口未就绪"))}</strong><span>${this.escape(error)}</span></div>` : ""}<div class="hrms-attendance-api-notice"><strong>${this.escape(__("已连接考勤判断"))}</strong><span>${this.escape(__("固定格式字段用于自动判断；备注和无法可靠结构化的文字仅作为审核依据，不会被当成任意公式执行。"))}</span></div><div class="hrms-attendance-result-controls"><span class="text-muted">${this.escape(__("当前规则版本：{0}；共 {1} 条", [data.rule_version || "--", rows.length]))}</span></div><div class="hrms-attendance-table-wrap"><table class="table table-bordered hrms-attendance-table"><thead><tr><th>${this.escape(__("状态"))}</th><th>${this.escape(__("规则"))}</th><th>${this.escape(__("匹配关键词"))}</th><th>${this.escape(__("基本班次"))}</th><th>${this.escape(__("平日加班"))}</th><th>${this.escape(__("周末/节日"))}</th><th>${this.escape(__("可取卡时段（上/下）"))}</th><th>${this.escape(__("夜班津贴规则"))}</th><th>${this.escape(__("来源"))}</th><th>${this.escape(__("操作"))}</th></tr></thead><tbody>${loading ? `<tr><td colspan="10" class="text-muted">${this.escape(__("正在读取班次规则..."))}</td></tr>` : rows.length ? rows.map(renderRow).join("") : `<tr><td colspan="10" class="text-muted">${this.escape(__("暂无公司班次规则，请首次导入排班表或手动新增。"))}</td></tr>`}</tbody></table></div></div>`;
	}

	bind_attendance_shift_rule_events(rows) {
		const body = this.body();
		body.querySelector("[data-import-shift-rules]")?.addEventListener("click", () => this.open_shift_rule_uploader());
		body.querySelector("[data-add-shift-rule]")?.addEventListener("click", () => this.open_shift_rule_dialog());
		body.querySelectorAll("[data-edit-shift-rule]").forEach((button) => button.addEventListener("click", () => this.open_shift_rule_dialog(rows.find((row) => row.name === button.dataset.editShiftRule) || {})));
	}

	open_shift_rule_uploader() {
		if (!this.ensure_company()) return;
		new frappe.ui.FileUploader({
			folder: "Home/Attachments",
			restrictions: { allowed_file_types: [".xlsx"] },
			on_success: (file) => this.call_processing_api("import_attendance_shift_rules", { company: this.company, file_url: file.file_url, preview_only: 1 }, {
				freeze: true,
				freeze_message: __("正在解析排班表并生成导入预览..."),
				on_success: (data) => this.open_shift_rule_import_preview(file.file_url, data || {}),
				on_error: (message) => frappe.msgprint(message),
			}),
		});
	}

	open_shift_rule_import_preview(fileUrl, data) {
		const issues = data.issues || [];
		const warnings = data.warnings || [];
		const sourceResolutions = data.source_resolutions || [];
		const specialNotes = data.special_notes || [];
		const rows = data.items || [];
		const sourceColumns = data.source_column_mapping || [];
		const mappedCells = (row) => {
			let raw = [];
			try { raw = JSON.parse(row.source_payload_json || "[]"); } catch (_error) { raw = []; }
			return `<details><summary>${this.escape(__("查看 B–U 原值与应用"))}</summary>${sourceColumns.map((item) => {
				const original = raw[item.column.charCodeAt(0) - 65];
				const applied = row[item.fieldname];
				return `<div><strong>${this.escape(`${item.column}${row.source_row} ${item.label}`)}</strong>：${this.escape(original === 0 ? "0" : original || "--")} → ${this.escape(applied === 0 ? "0" : applied || "--")}<br><small>${this.escape(item.application)}</small></div>`;
			}).join("")}</details>`;
		};
		const previewRows = rows.map((row) => `<tr><td>${this.escape(row.import_action || "")}</td><td>${this.escape(row.rule_name || "")}</td><td>${this.escape(row.match_tokens || "")}</td><td>${this.escape(`${row.shift_type_action || "--"}：${row.shift_type_name || "--"}`)}<br><small>${this.escape(row.shift_type_message || "")}</small></td><td>${this.escape(`${row.weekday_overtime_mode || "--"} / ${row.weekday_overtime_hours || 0} 小时`)}</td><td>${this.escape(row.source_row || "")}</td><td>${mappedCells(row)}</td></tr>`).join("");
		...
		const resolutionMarkup = sourceResolutions.length ? `<div class="alert alert-info"><strong>${this.escape(__("已确认的源表修正（原值仍保留）"))}</strong><ul>${sourceResolutions.map((item) => `<li>${this.escape(__("第 {0} 行：{1}", [item.source_row || "--", item.message || ""]))}</li>`).join("")}</ul></div>` : "";
		...
		const specialNoteMarkup = specialNotes.length ? `<div class="alert alert-info"><strong>${this.escape(__("源表特别说明（已确认的第 26 行时段参与食堂夜班规则，其余仅作审核依据）"))}</strong><ul>${specialNotes.map((item) => `<li>${this.escape(__("第 {0} 行：{1}", [item.source_row || "--", item.text || ""]))}</li>`).join("")}</ul></div>` : "";
		const dialog = new frappe.ui.Dialog({
			title: __("排班规则导入预览"), size: "extra-large",
			fields: [
				{ fieldname: "effective_from", fieldtype: "Date", label: __("本版规则生效日"), reqd: 1, default: frappe.datetime.get_today(), description: __("规则导入不按月份分批；生效日只用于保护历史结果和后续版本切换。") },
				...
				{ fieldtype: "HTML", options: `${issueMarkup}${warningMarkup}${resolutionMarkup}${manualMarkup}${specialNoteMarkup}<div class="alert alert-info">${this.escape(__("本文件是班别规则表，没有员工工号和排班日期。导入后可在实际排班中选用生成的系统班次；不会自动给员工排班。"))}</div><p>${this.escape(__("来源：{0} / {1}；版本 {2}。计算规则新增 {3} 条、更新 {4} 条；系统班次新增 {5} 个、保留待复核 {6} 个；治理规则：{7}。不会删除源表未出现的旧规则。", [data.source_file || "--", data.source_sheet || "--", data.source_version || "--", data.created_count || 0, data.updated_count || 0, data.shift_type_created_count || 0, data.shift_type_review_count || 0, data.scheduling_policy_action || "--"]))}</p><div class="hrms-attendance-table-wrap"><table class="table table-bordered"><thead><tr><th>${this.escape(__("规则动作"))}</th><th>${this.escape(__("班别"))}</th><th>${this.escape(__("匹配关键词"))}</th><th>${this.escape(__("系统班次联动"))}</th><th>${this.escape(__("平日加班"))}</th><th>${this.escape(__("源行"))}</th><th>${this.escape(__("字段映射"))}</th></tr></thead><tbody>${previewRows}</tbody></table></div>` },
			],
			primary_action_label: __("确认应用联动计划"),
			primary_action: () => {
				if (issues.length) { frappe.msgprint(__("排班表仍有结构问题，不能导入。")); return; }
				const values = dialog.get_values();
				if (!values) return;
				this.call_processing_api("import_attendance_shift_rules", { company: this.company, file_url: fileUrl, preview_only: 0, linkage_options: { ...values, source_checksum: data.source_checksum } }, {
					freeze: true, freeze_message: __("正在保存规则并同步系统班次..."),
					on_success: (result) => { dialog.hide(); frappe.show_alert({ message: result.notice || __("班次规则已导入。"), indicator: "green" }); this.refresh_attendance_rule_view(); },
					on_error: (message) => frappe.msgprint(message),
				});
			},
		});
	...
	open_scheduling_policy_dialog(existing = {}) {
		const dialog = new frappe.ui.Dialog({
			title: existing.name ? __("编辑排班治理规则") : __("新增排班治理规则"),
			size: "extra-large",
			fields: [
				{ fieldname: "policy_name", fieldtype: "Data", label: __("排班规则名称"), reqd: 1, default: existing.policy_name || "" },
				{ fieldname: "enabled", fieldtype: "Check", label: __("启用"), default: existing.enabled ?? 1 },
				{ fieldname: "priority", fieldtype: "Int", label: __("优先级"), default: existing.priority ?? 100, description: __("员工范围优先于部门，部门优先于全公司；同范围再按优先级选择。") },
				{ fieldtype: "Section Break", label: __("适用范围与周期") },
				{ fieldname: "scope_type", fieldtype: "Select", label: __("适用范围"), options: "全公司\n部门\n员工", reqd: 1, default: existing.scope_type || "全公司" },
				{ fieldname: "scope_values", fieldtype: "Small Text", label: __("适用对象"), default: existing.scope_values || "", depends_on: "eval:doc.scope_type!='全公司'", mandatory_depends_on: "eval:doc.scope_type!='全公司'", description: __("部门填写部门名称；员工只填写公司工号。多项用 | 分隔。") },
				{ fieldname: "effective_from", fieldtype: "Date", label: __("生效开始日"), reqd: 1, default: existing.effective_from || frappe.datetime.get_today() },
				{ fieldname: "effective_to", fieldtype: "Date", label: __("生效结束日"), default: existing.effective_to || "" },
				{ fieldtype: "Section Break", label: __("排班合规性") },
				{ fieldname: "allow_workday", fieldtype: "Check", label: __("工作日可排班"), default: existing.allow_workday ?? 1 },
				{ fieldname: "max_workday_hours", fieldtype: "Float", label: __("工作日排班上限（小时）"), default: existing.max_workday_hours ?? 24 },
				{ fieldname: "allow_restday", fieldtype: "Check", label: __("休息日可排班"), default: existing.allow_restday ?? 1 },
				{ fieldname: "max_restday_hours", fieldtype: "Float", label: __("休息日排班上限（小时）"), default: existing.max_restday_hours ?? 24 },
				{ fieldname: "allow_holiday", fieldtype: "Check", label: __("节假日可排班"), default: existing.allow_holiday ?? 1 },
				{ fieldname: "max_holiday_hours", fieldtype: "Float", label: __("节假日排班上限（小时）"), default: existing.max_holiday_hours ?? 24 },
				{ fieldname: "max_daily_hours", fieldtype: "Float", label: __("每日累计排班上限（小时）"), default: existing.max_daily_hours ?? 24 },
				{ fieldname: "calendar_weekend_mode", fieldtype: "Select", label: __("普通周六日考勤口径"), options: "休息日加班口径\n按工作日考勤", reqd: 1, default: existing.calendar_weekend_mode || "休息日加班口径", description: __("休息日加班口径会排除普通周末的标准工时、请假、旷工、迟到和早退，只核对打卡及加班；工作日、调班、补班仍按工作日。") },
				{ fieldtype: "Section Break", label: __("一天多班与连班") },
				{ fieldname: "allow_multiple_shifts", fieldtype: "Check", label: __("允许一天多班次"), default: existing.allow_multiple_shifts || 0 },
				{ fieldname: "merge_consecutive_shifts", fieldtype: "Check", label: __("连续班次合并取卡"), default: existing.merge_consecutive_shifts || 0, depends_on: "allow_multiple_shifts" },
				{ fieldname: "consecutive_gap_minutes", fieldtype: "Int", label: __("连班最大间隔（分钟）"), default: existing.consecutive_gap_minutes || 0, depends_on: "merge_consecutive_shifts" },
				{ fieldtype: "Section Break", label: __("排班修改与未排班打卡") },
				{ fieldname: "past_change_months", fieldtype: "Int", label: __("可修改过去月数"), default: existing.past_change_months ?? 6 },
				{ fieldname: "future_change_days", fieldtype: "Int", label: __("可修改未来天数"), default: existing.future_change_days ?? 365 },
				{ fieldname: "allow_change_after_checkin", fieldtype: "Check", label: __("已有打卡后允许改班"), default: existing.allow_change_after_checkin ?? 1 },
				{ fieldname: "allow_expired_unscheduled_change", fieldtype: "Check", label: __("逾期未排班允许补排"), default: existing.allow_expired_unscheduled_change ?? 1 },
				{ fieldname: "unscheduled_punch_mode", fieldtype: "Select", label: __("未排班打卡方式"), options: "不允许打卡\n选择班次打卡\n直接打卡\n选择班次打卡或直接打卡", default: existing.unscheduled_punch_mode || "选择班次打卡或直接打卡" },
				{ fieldname: "remarks", fieldtype: "Small Text", label: __("备注"), default: existing.remarks || "" },
			],
			primary_action_label: __("保存规则"),
			primary_action: (values) => {
				if (values.effective_to && values.effective_to < values.effective_from) {
					frappe.msgprint(__("生效结束日不能早于生效开始日。"));
					return;
				}
				if (values.merge_consecutive_shifts && !values.allow_multiple_shifts) {
					frappe.msgprint(__("开启连班合并前，必须先允许一天多班次。"));
					return;
				}
				this.call_processing_api("upsert_attendance_scheduling_policy", {
					company: this.company,
					policy: { ...existing, ...values },
				}, {
					freeze: true,
					freeze_message: __("正在保存排班治理规则..."),
					on_success: (data) => {
						dialog.hide();
						frappe.show_alert({ message: data.notice || __("排班治理规则已保存。"), indicator: "green" });
						this.refresh_attendance_rule_view();
					},
					on_error: (message) => frappe.msgprint(message),
				});
			},
	...
	split_schedule_ranges(value = "") {
		...
		for (let index = 0; index + 1 < matches.length; index += 2) {
			const start = `${String(Number(matches[index][1])).padStart(2, "0")}:${matches[index][2]}`;
			const end = `${String(Number(matches[index + 1][1])).padStart(2, "0")}:${matches[index + 1][2]}`;
			const between = text.slice(matches[index].index + matches[index][0].length, matches[index + 1].index);
			ranges.push(`${start}-${between.includes("次日") ? "次日" : ""}${end}`);
		}
		return ranges;
	}

	normalize_schedule_range(value, label, required = false) {
		...
		for (const match of matches) {
			if (Number(match[1]) === 24 && Number(match[2]) !== 0) throw new Error(__("24点只能写成 24:00。"));
		}
	...
	open_shift_rule_dialog(existing = {}) {
		const tokens = String(existing.match_tokens || "").split("|").map((value) => value.trim()).filter(Boolean);
		const shiftKeywords = ["白班", "夜班", "中班"];
		const shiftKeyword = tokens.length > 1 ? (tokens.find((value) => shiftKeywords.includes(value)) || "") : "";
		const primaryToken = tokens.find((value) => value !== shiftKeyword) || tokens[0] || "";
		const extraTokens = tokens.filter((value) => value !== primaryToken && value !== shiftKeyword).join("|");
		const basicRanges = this.split_schedule_ranges(existing.basic_time);
		const weekdayRanges = this.split_schedule_ranges(existing.weekday_overtime_time);
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldname: "dingtalk_attendance_groups", fieldtype: "Small Text", label: __("钉钉考勤组"), default: existing.dingtalk_attendance_groups || "", description: __("多个别名用 | 分隔；与下方钉钉班次成对命中时优先。") },
				{ fieldname: "dingtalk_shift_aliases", fieldtype: "Small Text", label: __("钉钉班次别名"), default: existing.dingtalk_shift_aliases || "", description: __("填钉钉实际班次名，多个用 | 分隔；后缀时间无需重复填写。") },
				{ fieldname: "enabled", fieldtype: "Check", label: __("启用"), default: existing.enabled ?? 1 },
				{ fieldname: "sequence", fieldtype: "Int", label: __("顺序"), default: existing.sequence || 0 },
				{ fieldname: "effective_from", fieldtype: "Date", label: __("生效日期"), default: existing.effective_from || "" },
				{ fieldname: "shift_type", fieldtype: "Link", options: "Shift Type", label: __("绑定系统班次"), default: existing.shift_type || "", description: __("班次分配使用的 Shift Type；从排班表联动导入时会自动填写。") },
				{ fieldtype: "Section Break", label: __("班次时间（固定格式）") },
				{ fieldname: "basic_time_primary", fieldtype: "Data", label: __("基本班次时段"), reqd: 1, default: basicRanges[0] || "", description: __("HH:MM-HH:MM；跨日会自动规范为 HH:MM-次日HH:MM。") },
				{ fieldname: "basic_time_secondary", fieldtype: "Data", label: __("第二基本时段"), default: basicRanges[1] || "", description: __("仅分段班次填写，例如烧饭阿姨夜班。") },
				{ fieldname: "weekday_overtime_time_fixed", fieldtype: "Data", label: __("平日加班时段"), default: weekdayRanges[0] || "" },
				{ fieldname: "weekend_overtime_time_fixed", fieldtype: "Data", label: __("周末加班时段"), default: weekendRanges[0] || "" },
				...
				{ fieldname: "holiday_overtime_mode", fieldtype: "Select", label: __("节日加班是否需申请"), options: "\n不提交加班单\n加班单\n无", default: existing.holiday_overtime_mode || "", description: __("明确标为节假日且匹配本班次时，有打卡无有效加班单会进入异常；此设置不自动生成节日计薪工时。") },
				...
				{ fieldname: "suggested_positions", fieldtype: "Small Text", label: __("建议岗位"), default: existing.suggested_positions || "" },
				{ fieldname: "punch_in_range_fixed", fieldtype: "Data", label: __("可取上班卡时段"), reqd: 1, default: this.split_schedule_ranges(existing.punch_in_range)[0] || "", description: __("超出该时段的上班卡不会参与迟到计算，并进入异常核对。") },
				{ fieldname: "punch_out_range_fixed", fieldtype: "Data", label: __("可取下班卡时段"), reqd: 1, default: this.split_schedule_ranges(existing.punch_out_range)[0] || "", description: __("支持跨日和 24:00；超出时不会参与加班计算。") },
				{ fieldname: "remarks", fieldtype: "Small Text", label: __("备注"), default: existing.remarks || "" },
			],
			primary_action_label: __("保存规则"),
			primary_action: (values) => {
				try {
					const primary = this.normalize_schedule_range(values.basic_time_primary, __("基本班次时段"), true);
					...
					this.call_processing_api("upsert_attendance_shift_rule", { company: this.company, rule }, {
						...
						on_success: (data) => { dialog.hide(); frappe.show_alert({ message: data.notice || __("班次规则已保存。"), indicator: "green" }); this.refresh_attendance_rule_view(); },
						on_error: (message) => frappe.msgprint(message),
					});
	...
	load_department_mappings() {
		const body = this.body();
		body.innerHTML = this.render_department_mappings([], true);
		if (!this.ensure_company()) return;
		this.call_processing_api("list_department_mappings", { company: this.company }, {
			on_success: (data) => { body.innerHTML = this.render_department_mappings(data.items || [], false); this.bind_department_mapping_events(); },
			on_error: (message) => { body.innerHTML = this.render_department_mappings([], false, message); this.bind_department_mapping_events(); },
		});
	}

	render_department_mappings(rows = [], loading = false, error = "") {
		const renderRow = (row) => `<tr><td>${this.escape(row.source_department || "--")}</td><td>${this.escape(row.target_department || "--")}</td><td>${this.status_badge(row.status || (row.enabled ? "已启用" : "已停用"))}</td><td>${this.escape(`${row.modified_by || "--"} ${row.modified || ""}`)}</td><td><button class="btn btn-default btn-xs" data-edit-department-mapping data-source-department="${this.escape(row.source_department || "")}" data-target-department="${this.escape(row.target_department || "")}" data-enabled="${row.enabled ? "1" : "0"}">${this.escape(__("编辑"))}</button></td></tr>`;
		return `<div class="hrms-attendance-section"><div class="hrms-attendance-list-head"><div><h3>${this.escape(__("部门映射"))}</h3><small>${this.escape(__("将已经确认的钉钉旧部门映射到当前花名册部门。映射只用于后续重新上传并加工的忘打卡批次，不改写已有来源或审核历史。"))}</small></div><button class="btn btn-primary btn-sm" data-add-department-mapping>${this.escape(__("新增部门映射"))}</button></div>${error ? `<div class="hrms-attendance-api-notice"><strong>${this.escape(__("接口未就绪"))}</strong><span>${this.escape(error)}</span></div>` : ""}<div class="hrms-attendance-table-wrap"><table class="table table-bordered hrms-attendance-table"><thead><tr><th>${this.escape(__("钉钉原部门"))}</th><th>${this.escape(__("花名册目标部门"))}</th><th>${this.escape(__("状态"))}</th><th>${this.escape(__("最近修改"))}</th><th>${this.escape(__("操作"))}</th></tr></thead><tbody>${loading ? `<tr><td colspan="5" class="text-muted">${this.escape(__("正在读取部门映射..."))}</td></tr>` : rows.length ? rows.map(renderRow).join("") : `<tr><td colspan="5" class="text-muted">${this.escape(__("暂无部门映射；部门不一致记录会继续保留在异常队列。"))}</td></tr>`}</tbody></table></div></div>`;
	}

	bind_department_mapping_events() {
		const body = this.body();
		body.querySelector("[data-add-department-mapping]")?.addEventListener("click", () => this.open_department_mapping_dialog());
		body.querySelectorAll("[data-edit-department-mapping]").forEach((button) => button.addEventListener("click", () => this.open_department_mapping_dialog({ source_department: button.dataset.sourceDepartment, target_department: button.dataset.targetDepartment, enabled: Number(button.dataset.enabled) })));
	}

	open_department_mapping_dialog(existing = {}) {
		const dialog = new frappe.ui.Dialog({
			title: existing.source_department ? __("编辑部门映射") : __("新增部门映射"),
			fields: [
				{ fieldname: "source_department", fieldtype: "Data", label: __("钉钉原部门"), default: existing.source_department || "", reqd: 1 },
				{ fieldname: "target_department", fieldtype: "Link", options: "Department", label: __("花名册目标部门"), default: existing.target_department || "", reqd: 1 },
				{ fieldname: "enabled", fieldtype: "Check", label: __("启用"), default: existing.enabled ?? 1 },
			],
			primary_action_label: __("保存映射"),
			primary_action: (values) => this.call_processing_api("upsert_department_mapping", { company: this.company, ...values }, {
				freeze: true,
				freeze_message: __("正在保存部门映射..."),
				on_success: (data) => { dialog.hide(); frappe.show_alert({ message: data.notice || __("部门映射已保存。"), indicator: "green" }); this.load_department_mappings(); },
				on_error: (message) => frappe.msgprint(message),
			}),
		});
		dialog.show();
	...
	get_final_source_checks(batch = {}) {
		...
		return this.final_required_sources.map((source) => {
			...
			if (external) {
				const status = external.status || (external.ready ? "已就绪" : "未就绪");
				return {
					...source,
					...external,
					// Older API responses reported confirmed records as “已确认”, while
					// newer responses use “已就绪”.  Both describe a completed source;
					// do not leave the generation button disabled during that rollout.
					ready: Boolean(external.ready ?? external.is_ready ?? ["已就绪", "已确认"].includes(status)),
					status,
	...
	get_first_signed_source_checks(batch = {}) {
	...
	render_monthly_support_imports(checks) {
		...
		return `
			<section class="hrms-attendance-monthly-support">
				<div class="hrms-attendance-list-head"><div><h3>${this.escape(__("考勤补充来源"))}</h3></div></div>
				<div class="hrms-attendance-monthly-support-grid">
					${this.monthly_support_sources.map((source) => {
						...
						const importErrors = Number(check.import_error_count || 0);
	...
	open_signed_final_reconciliation_uploader() {
		...
		new frappe.ui.FileUploader({
			folder: "Home/Attachments",
			restrictions: { allowed_file_types: [".xlsx"] },
			on_success: (file) => this.call_processing_api("reconcile_attendance_draft_with_signed_final", {
				company: this.company, attendance_month: this.attendance_month, file_url: file.file_url,
			}, {
				freeze: true,
				freeze_message: __("正在逐人与已签考勤终稿对账..."),
				on_success: (data) => {
					const result = data.reconciliation || {};
					frappe.show_alert({ message: __("已核对 {0} 人；考勤字段差异 {1} 人，金额字段差异 {2} 人，排除 {3} 人。", [result.matched_records || 0, result.attendance_changed_records ?? result.changed_records ?? 0, result.amount_changed_records ?? 0, result.excluded_records || 0]), indicator: "green" });
					this.load_monthly_final();
				},
				on_error: (message) => frappe.msgprint(message),
			}),
	...
	render_monthly_final() {
		...
		body.innerHTML = `<div class="hrms-attendance-monthly-workbench">${this.render_processing_notice()}${this.render_attendance_summary_markup(batch)}${this.render_monthly_support_imports(checks)}${this.render_monthly_final_markup(batch, checks)}</div>`;
		this.bind_attendance_summary_events(body);
	...
	process_monthly_support_file(sourceType) {
		this.call_processing_api("process_monthly_support_file", {
			...
			freeze: true,
			freeze_message: __("正在导入并校验月度补充来源..."),
			on_success: (data) => { frappe.show_alert({ message: data.metrics?.exception_rows ? __("导入校验发现 {0} 条错误，请查看结果并更正原文件后重新上传。", [data.metrics.exception_rows]) : __("导入校验通过，数据已自动生效。"), indicator: data.metrics?.exception_rows ? "orange" : "green" }); this.load_monthly_final(); },
			on_error: (message) => frappe.msgprint(message),
		});
	}
	...
	open_special_hours_manual_dialog() {
		...
		const dialog = new frappe.ui.Dialog({
			title: __("手动修改特殊工时"),
			fields: [
				{ fieldtype: "HTML", options: `<div class="hrms-attendance-dialog-note">${this.escape(__("按工号和日期修改该日特殊工时。保存后会重新汇总平日、休息日和节假日倍率；不会改写上传的 Excel，原值、原因、操作人和时间都会保留。工时填 0 可明确清零该日记录。"))}</div>` },
				{ fieldname: "employee_code", fieldtype: "Data", label: __("员工工号"), description: __("须为本月可进入终稿的考勤初稿人员"), reqd: 1 },
				{ fieldname: "attendance_date", fieldtype: "Date", label: __("日期"), reqd: 1 },
				{ fieldname: "hours", fieldtype: "Float", label: __("特殊工时（小时）"), reqd: 1 },
				{ fieldname: "reason", fieldtype: "Small Text", label: __("修改原因"), reqd: 1 },
			],
			primary_action_label: __("保存手动修改"),
			primary_action: (values) => {
				if (!String(values.attendance_date || "").startsWith(`${this.attendance_month}-`)) {
					frappe.msgprint(__("日期必须属于当前处理月份 {0}。", [this.attendance_month]));
					return;
				}
				this.call_processing_api("update_special_hours_manual_entry", {
					company: this.company,
					attendance_month: this.attendance_month,
					...
					freeze: true,
					freeze_message: __("正在保存特殊工时手动修改..."),
					on_success: (data) => {
						dialog.hide();
						frappe.show_alert({ message: data.notice || __("特殊工时手动修改已保存。"), indicator: "green" });
						this.load_monthly_final();
					},
					on_error: (message) => frappe.msgprint(message),
				});
	...
	generate_monthly_final_files() {
		this.call_processing_api(
			...
			{
				freeze: true,
				freeze_message: __("正在锁定同源数据并生成一次、二次签字版与财务版..."),
				on_success: (data) => {
					if (data?.blocked) {
					...
					frappe.show_alert({ message: __("一次、二次签字版与财务版已完成生成。"), indicator: "green" });
					this.load_monthly_final();
				},
				on_error: (message) => frappe.msgprint(message),
			},
	...
	generate_first_signed_file() {
		this.call_processing_api(
			"generate_first_signed_file",
			{ company: this.company, attendance_month: this.attendance_month, snapshot_version: this.processing_batch?.first_signed_outputs?.locked_snapshot_version || "" },
			{
				freeze: true,
				freeze_message: __("正在锁定考勤初稿与忘打卡并生成一次签字版..."),
				on_success: (data) => {
					if (data?.blocked) {
						frappe.msgprint({ title: __("一次签字版尚不能生成"), indicator: "orange", message: this.escape(data.reason || __("考勤初稿或忘打卡尚未确认。")) });
						this.load_monthly_final();
						return;
					}
					frappe.show_alert({ message: __("一次签字版已从考勤初稿与忘打卡的同一锁定版本生成。"), indicator: "green" });
					this.load_monthly_final();
				},
				on_error: (message) => frappe.msgprint(message),
			},
	...
	download_final_file(kind) {
		...
		const firstSignedOutputs = this.processing_batch?.first_signed_outputs || {};
		const fileUrl = kind === "first_signed" ? firstSignedOutputs.file_url : kind === "signed" ? outputs.signed_file_url : outputs.finance_file_url;
	...
	open_monthly_final_editor(kind = "signed") {
		this.call_processing_api("get_monthly_final_preview", {
			company: this.company, attendance_month: this.attendance_month, kind,
		}, {
			freeze: true,
			freeze_message: __("正在读取可编辑终稿..."),
			on_success: (data) => {
				if (!data?.available) {
					frappe.msgprint({ title: __("暂不能编辑终稿"), indicator: "orange", message: this.escape(data?.reason || __("请先生成终稿。")) });
					return;
				}
				const columns = data.columns || [];
				const editableFields = new Set(data.web_editable_fields || []);
				const rows = data.rows || [];
				const editableCell = (row, column) => {
					const value = String(row[column.field] ?? "");
					const attrs = `data-final-edit-field data-employee-code="${this.escape(row.employee_code || "")}" data-field-name="${this.escape(column.field)}" data-initial="${this.escape(value)}"`;
					if (column.field === "employee_signature" || column.field === "review_note") {
						return `<textarea class="form-control input-xs" rows="1" ${attrs}>${this.escape(value)}</textarea>`;
					}
					return `<input class="form-control input-xs" type="number" min="0" step="any" value="${this.escape(value)}" ${attrs}>`;
				};
				const table = `<div class="hrms-attendance-table-wrap"><table class="table table-bordered hrms-attendance-table"><thead><tr>${columns.map((column) => `<th>${this.escape(__(column.label))}</th>`).join("")}</tr></thead><tbody>${rows.map((row) => `<tr>${columns.map((column) => `<td>${editableFields.has(column.field) ? editableCell(row, column) : this.escape(row[column.field] ?? "")}</td>`).join("")}</tr>`).join("") || `<tr><td colspan="${columns.length || 1}" class="text-muted">${this.escape(__("当前锁定版本没有可编辑的数据。"))}</td></tr>`}</tbody></table></div>`;
				const dialog = new frappe.ui.Dialog({
					title: __("编辑第二次员工签字版（会同步重生成财务版）"),
					fields: [
						{ fieldtype: "HTML", fieldname: "final_editor", options: `<div class="hrms-attendance-dialog-note">${this.escape(__("此处修改的是第二次员工签字版的同源数据。保存后会记录原值和操作人，当前锁定版将不再作为有效版本；请重新锁定生成第二次员工签字版与财务版。平日、周末、节假日特殊工时必须按日期在“特殊工时 → 手动修改”中维护，以保留倍率依据。"))}</div>${table}` },
					],
					size: "extra-large",
					primary_action_label: __("保存修改并重新锁定"),
					primary_action: (values) => {
						const changesByCode = new Map();
						dialog.$wrapper[0].querySelectorAll("[data-final-edit-field]").forEach((input) => {
							if (String(input.value ?? "") === String(input.dataset.initial ?? "")) return;
							const code = input.dataset.employeeCode;
							const valuesForEmployee = changesByCode.get(code) || {};
							valuesForEmployee[input.dataset.fieldName] = input.value;
							changesByCode.set(code, valuesForEmployee);
						});
						const changes = [...changesByCode.entries()].map(([employee_code, valuesForEmployee]) => ({ employee_code, values: valuesForEmployee }));
						if (!changes.length) {
							frappe.msgprint(__("请先修改至少一个字段。"));
							return;
						}
						this.call_processing_api("update_monthly_final_rows", {
							company: this.company,
							attendance_month: this.attendance_month,
							changes: JSON.stringify(changes),
						}, {
							freeze: true,
							freeze_message: __("正在保存终稿网页修改..."),
							on_success: (result) => {
								dialog.hide();
								frappe.show_alert({ message: result.notice || __("网页修改已保存，请重新锁定终稿。"), indicator: "green" });
								this.load_monthly_final();
							},
							on_error: (message) => frappe.msgprint(message),
						});
				...
				dialog.$wrapper.addClass("hrms-monthly-final-editor-dialog");
				dialog.show();
			},
			on_error: (message) => frappe.msgprint(message),
		});
	...
	open_day_check_review(name) {
		frappe.call({
			...
			callback: (response) => {
				...
				this.daily_source = "";
				if (this.active_view === "daily-attendance") {
					this.daily_attendance_data = null;
					this.load_daily_attendance();
				} else {
					this.load_daily_checks();
	...
	open_rule_dialog(existing = {}) {
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{ fieldname: "priority", fieldtype: "Int", label: __("优先级"), default: existing.priority || 0, description: __("数字越小越靠前；仅影响显示和检查顺序。") },
				{ fieldname: "effective_from", fieldtype: "Date", label: __("生效日期"), default: existing.effective_from || "" },
	...
	run_attendance_rule_evaluation() {
		...
		frappe.call({
			...
			callback: (response) => {
				...
				if (this.active_view === "processing-rules") this.load_complete_attendance_rules();
				else this.load_attendance_rule_center();
			},
		});
	}
	...
	render_dingtalk_integration() {
		this.body().innerHTML = `
			<div class="hrms-attendance-section hrms-attendance-integration">
				<div class="hrms-attendance-list-head">
					<h3>${this.escape(__("钉钉集成"))}</h3>
					<div>
						<button class="btn btn-default btn-sm" data-action="dingtalk-settings">${this.escape(__("连接设置"))}</button>
		...
		`;
		this.body().querySelector("[data-action='dingtalk-settings']").addEventListener("click", () => this.open_dingtalk_configuration());
		this.body().querySelectorAll("[data-view]").forEach((button) => button.addEventListener("click", () => this.set_view(button.dataset.view)));
	...
	open_dingtalk_configuration() {
		frappe.call({
			method: "hrms.api.dingtalk_integration.get_dingtalk_connection_status",
			callback: (response) => {
				const result = response.message || {};
				const settings = result.settings || {};
				const defaults = result.defaults || {};
				const dialog = new frappe.ui.Dialog({
					 title: __("钉钉连接设置（扫码入职 + 考勤共用）"),
					 fields: [
						{ fieldtype: "HTML", options: `<div class="alert alert-info">${this.escape(__("这里配置一套钉钉连接：扫码入职和后续考勤拉取共用同一个 AppKey/AppSecret。两类数据只是在钉钉后台申请不同读取权限，系统内仍分别点击“同步扫码入职”和“手动拉取指定日期”。扫码入职不会自动拉取。"))}</div>` },
						{ fieldname: "company", fieldtype: "Link", options: "Company", label: __("同步公司"), default: settings.company || "", reqd: 1 },
						{ fieldname: "enabled", fieldtype: "Check", label: __("启用钉钉集成"), default: settings.enabled ? 1 : 0 },
						{ fieldname: "sync_mode", fieldtype: "Select", label: __("数据连接方式"), options: "内网服务器主动拉取API\nExcel导入（不调用钉钉）", default: settings.sync_mode || "内网服务器主动拉取API", description: __("选择 API 后，扫码入职和考勤按钮使用同一套钉钉连接。"), reqd: 1 },
						{ fieldname: "daily_sync_enabled", fieldtype: "Check", label: __("考勤每日自动同步（02:30，可选）"), description: __("每天同步到前天并复核最近 N 天；已锁定日期不会自动重开。默认关闭。"), default: settings.daily_sync_enabled ? 1 : 0 },
						{ fieldname: "sync_lookback_days", fieldtype: "Int", label: __("考勤回补天数"), description: __("可填 1–31，推荐 7。"), default: settings.sync_lookback_days || 7, depends_on: "eval:doc.daily_sync_enabled" },
						{ fieldtype: "Section Break", label: __("钉钉企业内部应用凭证") },
						{ fieldname: "client_id", fieldtype: "Data", label: __("AppKey / Client ID（必填）"), description: __("从钉钉开发者后台的企业内部应用复制。"), default: settings.client_id || defaults.client_id || "", reqd: 1 },
						{ fieldname: "client_secret", fieldtype: "Password", label: __("AppSecret / Client Secret（必填）"), description: result.client_secret_saved ? __("已保存；留空可保持不变。") : __("首次连接必填；只保存到服务器密码字段，不要发到聊天。") },
						{ fieldname: "agent_id", fieldtype: "Data", label: __("应用 AgentId（扫码入职必填）"), description: __("钉钉开发者后台企业内部应用的 AgentId；用于读取扫码填写的智能人事花名册字段。"), default: settings.agent_id || "", reqd: 1 },
						{ fieldtype: "HTML", options: `<div class="text-muted small">${this.escape(__("钉钉后台至少申请：智能人事—待入职员工管理权限、智能人事—个人信息读权限、考勤数据读权限。扫码入职使用智能人事花名册接口，不调用通讯录用户详情接口；无需回调地址或 Webhook。"))}</div>` },
					],
					primary_action_label: __("保存本地试运行配置"),
					primary_action: (values) => {
						const payload = {
							...values,
							local_gateway_enabled: 0,
							public_gateway_enabled: 0,
						};
						frappe.call({
							method: "hrms.api.dingtalk_integration.save_dingtalk_connection_settings",
							args: { settings_json: JSON.stringify(payload) },
							freeze: true,
							freeze_message: __("正在保存钉钉配置..."),
							callback: () => {
								dialog.hide();
								frappe.show_alert({ message: __("钉钉本地试运行配置已保存"), indicator: "green" });
			...
			error: (error) => frappe.msgprint({ title: __("无法读取钉钉配置"), indicator: "red", message: this.read_dingtalk_error(error) }),
		});
	}

	sync_dingtalk_preentries() {
		if (!this.ensure_company()) return;
		frappe.call({
			method: "hrms.api.dingtalk_integration.sync_preentry_employees_from_dingtalk",
			args: { company: this.company },
			freeze: true,
			freeze_message: __("正在拉取钉钉扫码入职记录..."),
			callback: (response) => {
				const result = response.message || {};
				frappe.show_alert({
					message: __("扫码入职同步完成：接收 {0}，已匹配 {1}，待匹配 {2}，失败 {3}", [result.received || 0, result.matched || 0, result.pending || 0, result.failed || 0]),
					indicator: result.failed ? "orange" : "green",
				});
				this.load_dingtalk_status();
			},
			error: (error) => frappe.msgprint({ title: __("扫码入职同步失败"), indicator: "red", message: this.read_dingtalk_error(error) }),
		});
	}

	sync_dingtalk_full_roster() {
		if (!this.ensure_company()) return;
		frappe.call({
	...
	test_dingtalk_connection() {
		frappe.call({
			method: "hrms.api.dingtalk_integration.fetch_access_token",
			freeze: true,
			freeze_message: __("正在测试钉钉连接..."),
			callback: (response) => {
				const result = response.message || {};
				frappe.show_alert({ message: __("连接成功，令牌有效至：{0}", [result.token_expires_at || "--"]), indicator: "green" });
				this.load_dingtalk_status();
			},
			error: (error) => frappe.msgprint({ title: __("钉钉连接失败"), indicator: "red", message: this.read_dingtalk_error(error) }),
		});
	}
	...
	open_dingtalk_local_pilot_dialog() {
		const dialog = new frappe.ui.Dialog({
			title: __("本地考勤试运行"),
			fields: [
				{ fieldname: "work_date", fieldtype: "Date", label: __("考勤日期"), default: this.attendance_date || frappe.datetime.add_days(frappe.datetime.get_today(), -2), reqd: 1, description: __("只能选择当前日期两天前及更早的日期。") },
				{ fieldname: "userids_json", fieldtype: "Small Text", label: __("钉钉 UserId"), description: __("每行一个；最多 5 名员工。请先完成员工同步并核对员工映射。"), reqd: 1 },
			],
			primary_action_label: __("提交试运行"),
			primary_action: (values) => {
				frappe.call({
					method: "hrms.api.dingtalk_integration.queue_dingtalk_local_pilot_sync",
					args: { ...values, company: this.company },
					freeze: true,
					freeze_message: __("正在提交本地考勤试运行..."),
					callback: (response) => {
						const result = response.message || {};
						dialog.hide();
						this.last_sync_log = result.sync_log || "";
						this.attendance_date = values.work_date;
						this.attendance_month = values.work_date.slice(0, 7);
						this.set_dingtalk_sync_progress({ step: "连接检查", title: __("本地试运行已进入后台队列"), message: __("未开放公网端口；系统只会拉取指定员工并生成可撤销的考勤草稿。"), sync_log: this.last_sync_log, summary: __("同步日期：{0}；指定员工：{1} 人。", [values.work_date, result.requested_user_count || 0]) });
						this.watch_dingtalk_attendance_sync(this.last_sync_log, values.work_date);
					},
					error: (error) => frappe.msgprint({ title: __("提交试运行失败"), indicator: "red", message: this.read_dingtalk_error(error) }),
				});
			},
		});
		dialog.show();
	...
	render_dingtalk_status(status) {
		...
		const cards = [
			...
			[raw.employee_roster || 0, "员工档案原始记录", "含全部返回字段和附件元数据"],
			[raw.preentry || 0, "扫码入职原始记录", "仅在点击“同步扫码入职”后拉取"],
	...
	sync_dingtalk_attendance_for_date() {
		if (!this.ensure_company()) return;
		const latestAllowedDate = frappe.datetime.add_days(frappe.datetime.get_today(), -2);
		frappe.prompt(
			[{
				fieldname: "work_date", fieldtype: "Date", label: __("同步考勤日期"),
				default: this.attendance_date || latestAllowedDate, reqd: 1,
				description: __("当前最多可同步到 {0}；目标日期必须已经过去两天。", [latestAllowedDate]),
			}],
			(values) => {
				if (String(values.work_date) > latestAllowedDate) {
					frappe.msgprint(__("当前最多可同步到 {0}。", [latestAllowedDate]));
					return;
				}
				this.preview_dingtalk_attendance_sync(values.work_date);
			},
			__("手工同步钉钉考勤"),
			__("检查同步范围"),
		);
	...
	preview_dingtalk_attendance_sync(workDate) {
		frappe.call({
			method: "hrms.api.dingtalk_integration.get_dingtalk_attendance_resync_preview",
			args: { work_date: workDate, company: this.company },
			freeze: true,
			freeze_message: __("正在检查历史同步、人工修改和锁定状态..."),
			callback: (response) => {
				const preview = response.message || {};
				if (preview.blocked) {
					frappe.msgprint({ title: __("不能重新同步"), message: this.escape(preview.blocked_reason || __("当前月份已锁定。")), indicator: "red" });
					return;
				}
				if (!preview.is_resync) {
					this.confirm_dingtalk_attendance_first_sync(workDate, preview);
					return;
				}
				this.confirm_dingtalk_attendance_resync(workDate, preview);
			},
			error: (error) => frappe.msgprint(this.read_dingtalk_error(error)),
		});
	}

	confirm_dingtalk_attendance_first_sync(workDate, preview) {
		const dialog = new frappe.ui.Dialog({
			title: __("确认同步钉钉考勤"),
			fields: [{
				fieldtype: "HTML",
				options: `<div class="alert alert-info"><strong>${this.escape(__("已完成同步范围检查，尚未提交任务。"))}</strong><br>${this.escape(__("日期：{0}；原始记录：{1} 条；API草稿：{2} 条；人工调整：{3} 条。", [workDate, preview.raw_records || 0, preview.api_rows || 0, preview.manual_rows || 0]))}<br>${this.escape(__("确认后只会生成可审核的日考勤草稿和异常队列，不会修改员工主档或薪资。"))}</div>`,
			}],
			primary_action_label: __("确认开始同步"),
			primary_action: () => {
				dialog.hide();
				this.run_dingtalk_attendance_sync(workDate, { preview });
			},
		});
		dialog.show();
	}

	confirm_dingtalk_attendance_resync(workDate, preview) {
		const closureStatus = preview.daily_closure?.status || __("未闭环");
		const previousTime = preview.previous_sync?.finished_at || __("未记录");
		const dialog = new frappe.ui.Dialog({
			title: __("确认重新同步钉钉考勤"),
			fields: [
				{
					fieldtype: "HTML",
					options: `<div class="alert alert-warning"><strong>${this.escape(__("这次操作会拉取钉钉最新数据并比较新旧结果。"))}</strong><br>${this.escape(__("日期：{0}；上次完成：{1}；API草稿：{2} 条；人工调整：{3} 条；日闭环：{4}。", [workDate, previousTime, preview.api_rows || 0, preview.manual_rows || 0, closureStatus]))}<br>${this.escape(__("人工调整不会被覆盖；发生变化时当天将回到待校验状态。"))}</div>`,
				},
				{ fieldtype: "Small Text", fieldname: "resync_reason", label: __("重新同步原因"), reqd: 1, description: __("例如：钉钉中补录换班、补卡或审批状态发生变化。") },
				{
					fieldtype: "Check", fieldname: "reopen_locked_day", label: __("如当天已锁定，授权重开并重新进入复核"),
					default: 0, hidden: preview.daily_locked ? 0 : 1,
					description: __("只有具备考勤审批权限的人员可以执行；月度已锁定时仍会阻止。"),
				},
			],
			primary_action_label: __("确认重新同步"),
			primary_action: (values) => {
				if (preview.daily_locked && !values.reopen_locked_day) {
					frappe.msgprint(__("当天已锁定，请确认重开后再重新同步。"));
					return;
				}
				dialog.hide();
				this.run_dingtalk_attendance_sync(workDate, {
					preview,
					resync_reason: values.resync_reason,
					reopen_locked_day: values.reopen_locked_day,
				});
			},
		});
		dialog.show();
	...
	run_dingtalk_attendance_sync(workDate, options = {}) {
		...
		frappe.call({
			...
			args: {
				work_date: workDate,
				company: this.company,
				resync_reason: options.resync_reason || "",
				reopen_locked_day: options.reopen_locked_day ? 1 : 0,
			},
			callback: (response) => {
				const queued = response.message || {};
				this.last_sync_log = queued.sync_log || "";
				this.set_attendance_sync_progress({ step: "连接检查", title: queued.is_resync ? __("钉钉考勤重新同步已进入后台队列") : __("钉钉考勤已进入后台队列"), message: queued.duplicate ? __("该日期已有同步任务，已继续跟踪原任务。") : __("可继续浏览其他页面；完成后可在“钉钉同步记录”定位本次日期。"), sync_log: this.last_sync_log, summary: __("同步日期：{0}", [workDate]) });
	...
	watch_dingtalk_attendance_sync(syncLog, workDate, attempt = 0) {
		...
		frappe.call({
			...
			callback: (response) => {
				...
				const summary = __("接收 {0}；新增 {1}；变化 {2}；未变化 {3}；人工修改冲突 {4}；失败 {5}。", [run.records_received || 0, run.records_created || 0, run.records_updated || 0, run.records_unchanged || 0, run.manual_conflicts || 0, run.records_failed || 0]);
				...
				const draftChanges = Number(run.records_created || 0) + Number(run.records_updated || 0) + Number(run.records_unchanged || 0);
				const reviewMessage = Number(run.manual_conflicts || 0) > 0
					? __("新数据已保留，人工调整未被覆盖；请先处理冲突并重新校验当天。")
					: !received
						? __("没有接收到钉钉考勤用户数据，不会生成审核队列；请检查员工映射和钉钉权限。")
						: !draftChanges
							? __("已接收 {0} 名员工，但没有形成可审核的日考勤草稿；请在原始记录或打卡记录中检查明细。", [received])
					: Number(run.records_updated || 0) > 0
						? __("钉钉数据发生变化，当天已回到待校验状态。")
						: __("同步完成；没有发现会改变日考勤草稿的数据。 ");
				this.set_attendance_sync_progress({ step: cancelled || failed ? "拉取原始数据" : "生成异常队列", title: cancelled ? __("本次钉钉同步已撤销") : failed ? __("钉钉同步未完成") : __("钉钉考勤同步完成，等待人事审核"), done: !failed, failed, message: cancelled ? __("已撤销本次生成的考勤草稿；原始数据保留用于审计。") : failed ? __("请在同步记录查看失败原因；不会影响月度或薪资。") : reviewMessage, summary, error: failed ? run.error_message || "" : "" });
				this.load_dashboard_summary();
				if (this.active_view === "dingtalk") this.load_dingtalk_status();
				else if (["daily", "exceptions", "clock-records"].includes(this.active_view)) this.load_active_view();
			},
		});
	}
	...
	render_dingtalk_sync_logs(rows) {
		const table = this.body().querySelector("[data-table]");
		if (!table) return;
		table.innerHTML = `<div class="hrms-attendance-table-wrap"><table class="table table-bordered hrms-attendance-table"><thead><tr><th>${this.escape(__("同步日期"))}</th><th>${this.escape(__("状态"))}</th><th>${this.escape(__("接收/新增/变化/冲突"))}</th><th>${this.escape(__("草稿批次"))}</th><th>${this.escape(__("提交/完成时间"))}</th><th>${this.escape(__("原因/说明"))}</th><th>${this.escape(__("操作"))}</th></tr></thead><tbody>${rows.length ? rows.map((row) => {
			const statusClass = ["失败", "部分失败"].includes(row.status) ? "is-warning" : row.status === "已撤销" ? "is-pending" : "";
			const note = [row.resync_reason ? __("重新同步：{0}", [row.resync_reason]) : "", row.error_message || ""].filter(Boolean).join("；") || "-";
			return `<tr><td>${this.escape(row.business_date || "-")}</td><td><span class="hrms-attendance-status ${statusClass}">${this.escape(row.status || "-")}</span></td><td>${this.escape(`${row.records_received || 0} / ${row.records_created || 0} / ${row.records_updated || 0} / ${row.manual_conflicts || 0}`)}</td><td>${this.escape(row.batch || "-")}</td><td>${this.escape(row.finished_at || row.started_at || "-")}</td><td class="hrms-attendance-long-cell">${this.escape(note)}</td><td><button class="btn btn-default btn-xs" data-open-sync-date="${this.escape(row.business_date || "")}">${this.escape(__("查看当天"))}</button>${row.can_cancel ? ` <button class="btn btn-default btn-xs" data-cancel-dingtalk-sync="${this.escape(row.name)}">${this.escape(__("撤销"))}</button>` : ""}</td></tr>`;
	...
	seed_attendance_custom_rules() {
		frappe.call({
			...
			callback: () => this.refresh_attendance_rule_view(),
		});
	}
