(function () {
	const FORM_TYPES = {
		employee_talk: { label: "员工谈话表", title_label: "谈话标题", material_type: "employee_talk_form" },
		employee_transfer_application: { label: "员工职务调动申请表", title_label: "调动标题", material_type: "employee_transfer_application" },
		reward_punishment_report: { label: "奖惩提报单", title_label: "奖惩标题", material_type: "reward_punishment_report" },
	};
	...
	class EmployeeFormSubmitPage {
		...
		show() {
			if (frappe.get_route()[1] === "records") {
				...
				this.page.set_title(__(`${this.form.label}录入记录`));
				this.render_records_page();
				return;
			}
			this.page.set_primary_action(__("录入记录"), () => this.show_records());
			this.page.set_title(__(this.form.label));
			this.render();
		}
		...
		this.wrapper.innerHTML = `
			<style>
				...
				.hrms-employee-form-submit__title { max-width: 680px; margin-top: 16px; }
				.hrms-employee-form-submit__title label { display: block; font-weight: 600; margin-bottom: 6px; }
				.hrms-employee-form-submit__title input { width: 100%; }
				.hrms-employee-form-submit__error { color: #b42318; margin-top: 6px; }
		...
		render_selected() {
			...
			const error = this.upload_error ? `<div class="hrms-employee-form-submit__error">${this.escape(this.upload_error)}</div>` : "";
			return `<div class="hrms-employee-form-submit__title"><label for="hrms-employee-form-title">${__(this.form.title_label)}<span class="text-danger"> *</span></label><input id="hrms-employee-form-title" class="form-control" data-role="form-title" maxlength="140" value="${this.escape(this.title)}" placeholder="${this.escape(__("请输入{0}", [this.form.title_label]))}">${error}</div><div class="hrms-employee-form-submit__selected"><span class="hrms-employee-form-submit__identity"><span class="hrms-employee-form-submit__photo">${this.render_employee_photo(employee)}</span><span><strong>${this.escape(employee.employee_name || "未填写姓名")}</strong><small>${this.escape(employee.employee_code || "未填写工号")} · ${this.escape(employee.department || "未填写部门")} · ${this.escape(employee.designation || "未填写岗位")}</small></span></span><button type="button" class="btn btn-primary" data-action="upload">${__("拍照/上传并提交")}</button></div>${success}`;
		}
		...
		bind_events() {
			...
			this.wrapper.querySelector("[data-role='form-title']")?.addEventListener("input", (event) => { this.title = event.currentTarget.value; this.upload_error = ""; });
		...
		upload() {
			if (!this.selected_employee) return;
			this.title = String(this.wrapper.querySelector("[data-role='form-title']")?.value || this.title || "").trim();
			if (!this.title) {
				this.upload_error = __("请输入{0}", [this.form.title_label]);
				this.render();
				return;
			}
			...
			new frappe.ui.FileUploader({ doctype: "Employee", docname: this.selected_employee.name, allow_multiple: false, allow_take_photo: true, allow_web_link: false, disable_file_browser: true, restrictions: { allowed_file_types: [".jpg", ".jpeg", ".png", ".webp", ".pdf"], max_file_size: 10 * 1024 * 1024 }, on_success: (file, response) => {
				...
				if (!file_url) return;
				frappe.call({ method: "hrms.api.employee_form_entry.archive_employee_form_attachment", args: { employee: this.selected_employee.name, form_type: this.form_type, file_url, title }, freeze: true, freeze_message: __("正在提交员工表单…") }).then(() => {
				...
				}, (error) => { this.upload_error = error?.message || __("员工表单提交失败，请重试"); this.render(); });
			} });
		}
		...
		render_records_page() {
			this.wrapper.innerHTML = `
				<style>
					...
					.hrms-employee-form-record__field--title { grid-column: 1 / -1; }
