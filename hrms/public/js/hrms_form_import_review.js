...
(function () {
	...
	function addApprovalButton(frm, label, callback, group) {
		const capability = window.hrmsFormImport?.approvalCapabilityForTemplate(frm.doc.template_key);
		const button = frm.add_custom_button(label, () => {
			if (window.hrmsCapabilities?.require(capability)) callback();
		}, group);
	...
	frappe.ui.form.on("HRMS Form Import Row", {
		refresh(frm) {
			...
			if (canReview && frm.doc.review_status !== "已批准" && frm.doc.review_status !== "已驳回") {
				...
				addApprovalButton(frm, approveLabel, () => {
			...
			if (frm.doc.review_status === "已批准" && !frm.doc.target_name) {
				addApprovalButton(frm, __("生成正式草稿"), () => {
					frappe.confirm(
			...
			if (frm.doc.review_status === "已批准" && frm.doc.target_name && frm.doc.status !== "已提交生效") {
				addApprovalButton(frm, __("提交并生效"), () => {
					frappe.confirm(
