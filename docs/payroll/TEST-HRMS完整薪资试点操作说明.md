...
薪资异动和社保公积金缴费档案遵循永久审计规则，不会被重置命令删除；重复运行会复用同一测试员工、月份和已批准标准。若考勤已经锁定但薪资尚未完成，试点会复用该锁定终稿继续计算，不重复导入或解锁考勤。
...
| 社保公积金 | 通过独立缴费档案提交并审批个人和公司承担金额 | `HRMS Employee Contribution Change` |
| 福利扣款 | 学历、租房、宿舍、生产奖、提案、继续服务、所得税、水电、离职结算等 | `HRMS Payroll Welfare Source Record` |
| 变量导入 | 导入奖惩等非考勤变量；全勤奖由锁定考勤和全勤规则自动计算，禁止重复导入 | 变量批次、`HRMS Payroll Variable Record` |
...
- 异常队列必须包含：`忘打卡`、`早退`、`旷工`、`未申请加班`；工作日来源另须有迟到事实。早期已锁定试点若使用了周末迟到样例，会返回验证警告而不改写锁定记录。
