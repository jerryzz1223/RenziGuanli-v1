...
for (const marker of [
	...
	"文件夹部门（可包含下级部门）",
	"允许花名册归属",
	"enforce_yongxin_roster_assignment",
	"下级部门",
	"花名册归属员工",
...
for (const marker of [
	...
	'"employee_match_mode": "department" if kind in ROSTER_UNIT_KINDS else "assigned" if manual_people else "display" if kind in {"管理层", "分管"} else None',
