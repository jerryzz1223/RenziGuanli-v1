...
mustInclude(employee, 'if selected == "在职·正式":', "正式在职必须匹配雇佣类型、状态和转正字段。");
mustInclude(employee, 'elif selected == "在职·试用期":', "试用在职必须匹配雇佣类型、状态和转正字段。");
mustInclude(employee, 'elif selected == "退休返聘":', "返聘必须匹配雇佣类型和状态字段。");
mustInclude(employee, 'elif selected == "待离职":', "待离职必须匹配实际状态字段。");
mustInclude(employee, 'elif selected == "离职":', "离职必须匹配实际状态字段。");
mustInclude(employeeForm, 'function sync_employee_work_nature_dependent_fields(frm, work_nature', "工作性质必须同步离职日期的页面显示规则。");
...
mustInclude(employeeForm, '["relieving_date", "离职信息"]', "离职日期必须放入在职信息编辑区。");
...
mustInclude(employeeForm, 'frm.toggle_display("custom_work_nature", false)', "新建员工不得在表单中选择工作性质。");
mustInclude(employeeForm, 'frm.set_value("custom_work_nature", "在职·试用期")', "新建员工表单必须默认试用期。");
mustInclude(employeeForm, 'frm.toggle_display("custom_probation_months", false)', "旧的试用期月数输入必须从员工表单移除。");
mustInclude(employee, 'hrms_employee_roster_import', "服务端必须区分 Excel 导入与手工新建。");
mustInclude(template, '_retire_employee_fields(doc)', "旧的试用期月数字段必须停用但保留历史数据。");
