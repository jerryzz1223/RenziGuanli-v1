...
mustInclude(list, "apply_single_roster_filter(card, get_stored_roster_column_filter(), listview)", "状态卡必须使用渲染时的列表实例进行页内切换。");
...
if (list.includes("window.location.assign(")) throw new Error("状态卡不得整页重载");
mustInclude(list, "const source_records = custom_records", "不得用旧原生列表补回当前卡片数据");
